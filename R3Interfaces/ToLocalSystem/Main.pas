...
type
  ...
  public
    ...
    ApplIniName:string; // ini-файл
    UserADMIN:boolean; // legacy
    MasterXLSPath, // legacy
    MasterRPTPath:string; // legacy
    LoadPath:string; // каталог загрузки
    LoadFile:string; // маска загружаемого файла
    ArhFrom:string; // архивный каталог
    ListFields:string; // перечень полей
    BufferTable:string; // буферная таблица
    PostProcedure:string; // пост-процедура
    Load1,Load2,Load3,Load4,Load5:string; // возможные типы файлов
    ToLoad:string; // загружаемый тип
...
begin
  ...
  Information('Обработка файла '+filename,false);
  OracleQuery1.DeleteVariables;
  OracleQuery1.SQL.clear;

//Проверка загруженности файла
  ...
  if OracleQuery1.FieldAsInteger('acount')>0 then
      begin
      ApplSession.WriteToLog(amDefault,'MAIN',sesError,'Попытка загрузки файла, который уже был загружен. Файл '+ExtractFileName(filename)+' не будет обработан',0,'');
      result:=CONST_OK;
      exit;
      end;
  ...
  try
    ...
    try
      ...
      Information('Предобработка выполнена',false);
      Information('Занесение в БД',false);
      ORASQLEXPRESSION.Execute;
      ora_Session.Commit;
      Information('Занесение в БД выполнено',false);
      OracleQuery1.Clear;
      result:=CONST_OK;
    except
      on E: Exception do
      Begin
        Information(E.Message+#13#10+'Ошибка загрузки данных',true);
      end;
    end;
    CSVToDBMapper.Free;
  except
...
begin
  ...
  if (ParCnt>1) and (Par2='/AUTO') then
    begin
      ...
      Begin
        Information('Запуск пост обработки: '+PostProcedure,false);
        ...
        OracleQuery1.SQL.Add('END;');
         ...
         OracleQuery1.Execute;
         ora_Session.Commit;
         Information('Пост обработка выполнена',false);
         except
         on E: Exception do
            Begin
            Information(E.Message+#13#10+'Ошибка при пост обработке',true);
            end;
          end;
      end;
      Information('Транзакция завершена',false);
      Close;
      exit;
    end;
end;
  ...
  try
    ...
    with F_Env.ApplIni do
      begin
        ...
        ParCnt:=ParamCount;
        Par1:=AnsiUpperCase(ParamStr(1));
        // Считываем параметры загружаемого файла
        if (ParCnt>0) and
    ...
    Information('Установлено соединение с Oracle',false);
  except
    on E: Exception do
    Begin
      // Открыть сессию
      ApplSession.InitSession(amAuto,APPLICATION_NAME,SCHEMA_NAME,'MAIN',ApplIniName,'Сессия приложения',nil);
      Information(E.Message+#13#10+'Не удалось получить доступ к БД',true);
    end;
  end;
end;

...
var
  ...
  RunModest: string;
    ...
    // ÌÓÏÂ ÒÚÓÍË Ò ORDER BY

  // Á‡ÍÎ‡‰Í‡ ‰Îˇ ÔÂÂÏÂ˘ÂÌËˇ ÏÂÊ‰Û Ú‡·‡ÏË
...
////*****************‚˚·Ó “¿B*******************
////TODO -c¿«— -obeatlov: Œœ≈–¿÷»ﬂ ¬˚·Ó tab (ÔËıÓ‰, Â‡ÎËÁ‡ˆËˇ, ÓÒÚ‡ÚÓÍ?)
...
begin
  ...
  // PSV - ‰Îˇ ÙËÎ¸Ú‡ ÔÓ ÙËÎË‡ÎÛ
  q_Filials.Session:=f_main.ora_Session;
  q_Filials.Open;
  lc_Filial.KeyValue:=31; //œÓ ÛÏÓÎ˜‡ÌË˛ - ”ıÚ‡
...
begin
  inherited;
IF Application.MessageBox('¬€ Û‚ÂÂÌÌ˚, ˜ÚÓ ıÓÚËÚÂ Û‰‡ÎËÚ¸ ËÏÂÌÌÓ ˝ÚÛ Á‡ÔËÒ¸?', '¬ÌËÏ‡ÌËÂ', 1) = 1 then
begin

IF (azc_oper_all.operation <> 'À» ¿–ƒ') and (azc_oper_all.operation <> '√–”« ¬ œ”“»')  then
 begin
 name_ods:= ODS_Azc_realiz ;
 name_operation:= 'AZC_OPERATION';
 end
else
IF azc_oper_all.operation = 'À» ¿–ƒ' then
 begin
 name_ods:= ODS_likard;
 name_operation:= 'AZC_REALIZ_LIKARD';
 end
else
IF azc_oper_all.operation = '√–”« ¬ œ”“»' then
 begin
 name_ods:= ODS_good;
...
//*****************»«Ã≈Õ≈Õ»≈ Œœ≈–¿÷»»********************
//TODO -c¿«— -obeatlov: Œœ≈–¿÷»ﬂ ËÁÏÂÌÂÌËÂ
...
ix:= 0;
azc.RunModest:='EDIT';
IF ((azc_oper_all.operation = 'À» ¿–ƒ') and (ODs_likard.CountQueryHits>=1))
then ix:= 1;

IF ((azc_oper_all.operation = '√–”« ¬ œ”“»') and ( ODs_good.CountQueryHits>=1))
then ix:=1;

IF  (azc_oper_all.operation <> '√–”« ¬ œ”“»') and (azc_oper_all.operation <> 'À» ¿–ƒ')
...
if gdSelected in State then
    Background:=clNavy;
if ods_AZC_REALiz.FieldByName('FOR_AZC').AsString<>'¡≈« — »ƒŒ ' then
...
if Caption='NAME_1' then Caption:='Œ·˙ÂÍÚ';
if Caption='ABBR_NPR' then Caption:='œÓ‰ÛÍÚ';
if Caption='MONEY' then Caption:='—ÛÏÏ‡';
if Caption='VES_SUM' then Caption:='¬ÂÒ';
if Caption='VOL_SUM' then Caption:='Œ·˙ÂÏ';
end;
end;

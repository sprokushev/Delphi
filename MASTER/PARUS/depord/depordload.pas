...
Begin
frmMain.WriteEvent(Tag, 'Ôîðìà "Çàÿâêè â ïîïîäðàçäåëåíèÿ â Ïàðóñ"');
End;
...
//  oraPR17.SQL[16]:='and (m.planstru_id=95 or m.planstru_id=178 or m.poluch_id=2889 or (m.planstru_id=105 and m.poluch_id=2889))'
  ...
  Else
    oraPR17.SQL[16] := oraPR17.SQL[16];
  If rbSyk.Checked Then //Syktyvkar 2364 Àýðîïîðò 131 Àãðîñòðîéêîíñòðóêöèÿ
...
Begin
  ...
  //Ïîèñê êàòàëîãà äëÿ çàêàçà ïîñòàâùèêà
  ...
  oraDelCat.SetVariable(':sNAME','Æ/Ä ÓÍÏ');
  oraDelCat.Execute;
  ...
  While Not ss^.Eof Do
    Begin
      If rbArx.Checked Then
        Begin
          //Àðõàíãåëüñê
          oraInvIns.SetVariable(':sAGENT', 'Áîðîäèí È.Â.');
          oraInvIns.SetVariable(':sFACEACC', 'ÀÐÕÀÍ_ÍÁ_02/Ò');
          oraInvIns.SetVariable(':sACC_AGENT', 'Çàõâàòêèíà Å.À.');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÏÎÑÒÀÂÎÊ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÀÐÕÀÍÃÅËÜÑÊ');
          rn_subdiv:='ÀÐÕÀÍÃÅËÜÑÊ';
          oraInvIns.SetVariable(':sORD_PREF', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_NUMB', 'ÑÍÏ');
        End;
      If rbVor.Checked Then
        Begin
          //Âîðêóòà
          oraInvIns.SetVariable(':sAGENT', 'Êîëîòîâ Â.Í.');
          oraInvIns.SetVariable(':sFACEACC', 'ÂÎÐÊÓÒÀ_ÍÁ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÂÎÐÊÓÒÀ');
          rn_subdiv:='ÂÎÐÊÓÒÀ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'âîðê_í/á');
        End;
      If rbInta.Checked Then
        Begin
          //Inta
          oraInvIns.SetVariable(':sAGENT', 'Êàòèëîâ Í.È.');
          oraInvIns.SetVariable(':sFACEACC', 'ÈÍÒÀ_ÍÁ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÈÍÒÀ');
          rn_subdiv:='ÈÍÒÀ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'èíòà_í/á');
        End;
      If rbKp.Checked Then
        Begin
          //KnyagPogost
          oraInvIns.SetVariable(':sFACEACC', 'ÊÍßÆ-ÏÎÃ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÊÍßÆÏÎÃÎÑÒ_ÖÅÕ');
          rn_subdiv:='ÊÍßÆÏÎÃÎÑÒ_ÖÅÕ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'êíï_öåõ');
        End;
      If rbKos.Checked Then
        Begin
          //koslan
          oraInvIns.SetVariable(':sAGENT', 'Áóöèê È.À.');
          oraInvIns.SetVariable(':sFACEACC', 'ÊÎÑËÀÍ_ÍÁ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÊÎÑËÀÍ');
          rn_subdiv:='ÊÎÑËÀÍ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'êîñëàí_í/á');
        End;
      If rbPech.Checked Then
        Begin
          //Pechera
          oraInvIns.SetVariable(':sAGENT', 'Ôèëèïïîâ Í.À.');
          oraInvIns.SetVariable(':sFACEACC', 'ÏÅ×ÎÐÀ_ÍÁ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÏÅ×ÎÐÀ');
          rn_subdiv:='ÏÅ×ÎÐÀ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'ïå÷îðà_í/á');
        End;
      If rbPAZS.Checked Then
        Begin
          //Pechera  AZS
          oraInvIns.SetVariable(':sAGENT', 'Ôèëèïïîâ Í.À.');
          oraInvIns.SetVariable(':sFACEACC', 'ÏÅ×ÎÐÀ_ÀÇÑ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÏÅ×_ ÀÇÑ');
          rn_subdiv:='ÏÅ×_ ÀÇÑ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'ïå÷îðà_àçñ');
        End;
      If rbSyk.Checked Then
        Begin
          //Syktyvkar
          oraInvIns.SetVariable(':sAGENT', 'Õàìçèí Ð.Ð.');
          oraInvIns.SetVariable(':sFACEACC', 'ÑÛÊÒ_ÍÁ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÑÛÊÒÛÂÊÀÐ');
          rn_subdiv:='ÑÛÊÒÛÂÊÀÐ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'ñûêò_í/á');
        End;
      If rbUs.Checked Then
        Begin
          //Usinsk
          oraInvIns.SetVariable(':sAGENT', 'Àáìàåâ Â.Ñ.');
          oraInvIns.SetVariable(':sFACEACC', 'ÓÑÈÍÑÊ_ÍÁ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÓÑÍ');
          rn_subdiv:='ÓÑÍ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'óñèíñê_í/á');
        End;
      If rbUh.Checked Then
        Begin
          //Uhta
          oraInvIns.SetVariable(':sFACEACC', 'ÓÕÒÀ_ÍÁ_01/Ò');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ÓÕÒÀ_ÍÁ');
          rn_subdiv:='ÓÕÒÀ_ÍÁ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'óõòà_í/á');
        End;
      If rbLabyt.Checked Then
        Begin
          //labytnangi
          oraInvIns.SetVariable(':sAGENT', 'Êóëüáèöêàÿ È.Í.');  //
          oraInvIns.SetVariable(':sFACEACC', '');
          oraInvIns.SetVariable(':sACC_SUBDIV', 'ÎÒÄÅË_ÌÀÐÊÅÒÈÍÃÀ');
          oraInvIns.SetVariable(':sSUBDIV', 'ßÍÀÎ');
          rn_subdiv:='ßÍÀÎ';
          oraInvIns.SetVariable(':sORD_NUMB', trim(ss^.fieldbyname('nom_zd').asString));
          oraInvIns.SetVariable(':sORD_PREF', 'ëàá_í/á');
        End;
      ...
      WriteEvent('Çàêà÷êà â Ïàðóñ çàÿâêè ¹ ' + ss^.fieldbyname('nom_zd').AsString);
      rn_sved := 0;
        ...
        Begin
          ...
          If (oraQ.RowCount > 0) Then
            Begin
              ...
              oraProps.SetVariable(':NRN', rn_sved);
              oraProps.SetVariable(':SUNITCODE', 'DepartmentsOrders');
              oraProps.SetVariable(':SCODE', 'Êîä ñòàíöèè');
              oraProps.SetVariable(':NFORMAT', 0);
              oraProps.SetVariable(':SSTR_VALUE', oraQ.FieldAsString(0));
              Try
                Begin
                  oraProps.Execute;
                  WriteEvent('Êîä ñòàíöèè çàãðóæåí');
                End;
              Except On E: EOracleError Do
                  ShowMessage(E.Message);
              End;
          ...
          oraProps.SetVariable(':NRN', rn_sved);
          oraProps.SetVariable(':SUNITCODE', 'DepartmentsOrders');
          oraProps.SetVariable(':SCODE', 'Ãðóçîïîëó÷àòåëè');
          oraProps.SetVariable(':NFORMAT', 0);
          oraProps.SetVariable(':SSTR_VALUE', trim(ss^.fieldbyname('agnfifo').asString));
          Try
            Begin
              oraProps.Execute;
              WriteEvent('Ãðóçîïîëó÷àòåëü çàãðóæåí');
            End;
          Except On E: EOracleError Do
              ShowMessage(E.Message);
          End;
          ...
          Try
            Begin
              oraInvsIns.Execute;
              WriteEvent('Ñïåöèôèêàöèÿ çàÿâêè çàãðóæåíà!');
              oraQuery.SQL.Clear;
              ...
              oraQ.SQL.Clear;
              oraQ.Session.Commit;
              WriteEvent('Çàêàç ïîäðàçäåëåíèé ñôîðìèðâàí óñïåøíî!');
              ...
              End;
              oraQ.Session.Commit;
              WriteEvent('Óòâåðæäåíà Çàêàç ïîäðàçäåëåíèé ¹ ' + ss^.fieldbyname('nom_zd').AsString + #13#10);
            End
          Except On E: EOracleError Do
              ShowMessage(E.Message);
          End
      ...
      End;
//************************** Çàêàç ïîñòàâùèêà****************************************************
      WriteEvent('Çàêà÷êà â Ïàðóñ çàêàçà ïîñòàâùèêà ¹' + ss^.fieldbyname('nom_zd').AsString + #13#10);
      rn_sved := 0;
      oraDelIns.ClearVariables;
      oraDelIns.SetVariable(':nCOMPANY', 2);
      oraDelIns.SetVariable(':nCRN', rn_crn2);
      oraDelIns.SetVariable(':sJUR_PERS', 'ÑÅÂÅÐÍÅÔÒÅÏÐÎÄÓÊÒ');
      oraDelIns.SetVariable(':sORD_DOCTYPE', 'ÇÀÊÏÎÑÒ');
      oraDelIns.SetVariable(':sORD_PREF', ss^.fieldbyname('nom_zd').asString);
      oraDelIns.SetVariable(':sORD_NUMB', '1');
      oraDelIns.SetVariable(':sAGENT', 'ËÓÊÎÉË-ÓÕÒÀÍÅÔÒÅÏÅÐÅ');
      ...
      oraDelIns.SetVariable(':sDISP_TYPE', 'Æ/Ä_ÒÐÀÍÇ');
      oraDelIns.SetVariable(':sPAY_TYPE', 'ÏÎ ÔÀÊÒÓ 22');
//      oraDelIns.SetVariable(':sTARIF', trim(ss^.fieldbyname('starif').asString));
      oraDelIns.SetVariable(':sCURRENCY', 'Ðóá.');
      oraDelIns.SetVariable(':sSTORE', 'ÓÕÒÀ_ÖÀ');
      oraDelIns.SetVariable(':sACC_AGENT', 'Êóëüáèöêàÿ È.Í.');
...
//      oraDelIns.SetVariable(':dPRICE_DATE', FormatDateTime('dd.mm.yyyy', ss^.fieldbyname('date_plan').asDateTime));
      ...
      Try
        Begin
          ...
          WriteEvent('Çàãîëîâîê Çàêàçà çàãðóæåí...' + #13#10);
          oraProps.ClearVariables;
          oraProps.SetVariable(':NCOMPANY', 2);
          oraProps.SetVariable(':NRN', rn_sved);
          oraProps.SetVariable(':SUNITCODE', 'DeliveryOrders');
          oraProps.SetVariable(':SCODE', 'ÏËÀÍ ÏÎÑÒÀÂÊÈ');
          oraProps.SetVariable(':NFORMAT', 0);
          oraProps.SetVariable(':SSTR_VALUE', trim(ss^.fieldbyname('planstru_id').asString));
          Try
            Begin
              oraProps.Execute;
              WriteEvent('Êîä Ïëàíà Ïîñòàâêè çàãðóæåí...');
            End;
          Except On E: EOracleError Do
              ShowMessage(E.Message);
          End;
          ...
          oraDelsIns.SetVariable(':sSTORE', 'ÓÕÒÀ_ÖÀ');
          ...
          Try
            Begin
              oraDelsIns.Execute;
              WriteEvent('Ñïåöèôèêàöèÿ çàêàçà ïîñòàâùèêó çàãðóæåíà...');
              ...
              oraQ.SQL.Clear;
              oraQ.Session.Commit;
              WriteEvent('Çàêàç ïîñòàâùèêó ñôîðìèðâàí óñïåøíî!');
              ...
              End;
              oraQ.Session.Commit;
              WriteEvent('Óòâåðæäåíà Çàêàç Ïîñòàâùèêó!' + #13#10);
						End
					Except On E: EOracleError Do
							ShowMessage(E.Message);
          End

object frmTTN17: TfrmTTN17
  Left = 222
  Top = 155
  Width = 827
  Height = 710
  Caption = #1056#1072#1089#1093#1086#1076#1085#1099#1077' '#1085#1072#1082#1083#1072#1076#1085#1099#1077' '#1087#1086#1090#1088#1077#1073#1080#1090#1077#1083#1103#1084' '#1074' '#1055#1072#1088#1091#1089
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Splitter3: TSplitter
    Left = 185
    Top = 84
    Height = 595
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 819
    Height = 84
    Align = alTop
    TabOrder = 0
    object BitBtn2: TBitBtn
      Left = 221
      Top = 30
      Width = 108
      Height = 47
      Action = acLoad
      Caption = #1047#1072#1075#1088#1091#1079#1080#1090#1100' '#1074' '#1055#1072#1088#1091#1089
      TabOrder = 0
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object GroupBox1: TGroupBox
      Left = 6
      Top = 5
      Width = 211
      Height = 74
      Caption = #1060#1080#1083#1100#1090#1088
      TabOrder = 1
      object Label1: TLabel
        Left = 5
        Top = 14
        Width = 52
        Height = 13
        Caption = #1047#1072' '#1087#1077#1088#1080#1086#1076
      end
      object Label2: TLabel
        Left = 9
        Top = 31
        Width = 6
        Height = 13
        Caption = #1089
      end
      object Label3: TLabel
        Left = 9
        Top = 54
        Width = 12
        Height = 13
        Caption = #1087#1086
      end
      object deBeg: TDateEdit
        Left = 27
        Top = 28
        Width = 83
        Height = 21
        DefaultToday = True
        NumGlyphs = 2
        TabOrder = 0
        Text = '17.03.2003'
      end
      object deEnd: TDateEdit
        Left = 27
        Top = 50
        Width = 83
        Height = 21
        DefaultToday = True
        NumGlyphs = 2
        TabOrder = 1
        Text = '17.03.2003'
      end
      object BitBtn1: TBitBtn
        Left = 120
        Top = 40
        Width = 81
        Height = 26
        Action = acFilter
        Caption = #1054#1090#1086#1073#1088#1072#1090#1100
        TabOrder = 2
        Glyph.Data = {
          
          00000000000000000000000000000000000000000000FF00FF00}
      end
      object rbGD: TRadioButton
        Left = 120
        Top = 8
        Width = 80
        Height = 17
        Caption = #1046'\'#1044' '#1090#1072#1088#1080#1092
        TabOrder = 3
      end
      object rbProd: TRadioButton
        Left = 120
        Top = 24
        Width = 80
        Height = 17
        Caption = #1055#1088#1086#1076#1091#1082#1090
        TabOrder = 4
      end
    end
  end
  object Panel2: TPanel
    Left = 188
    Top = 84
    Width = 631
    Height = 595
    Align = alClient
    TabOrder = 1
    object dbgTTN17_new: TDBGridEh
      Left = 1
      Top = 1
      Width = 629
      Height = 593
      Align = alClient
      AllowedOperations = []
      DataSource = dsTTN17_new
      Flat = False
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -11
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = []
      OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghFitRowHeightToText, dghAutoSortMarking, dghMultiSortMarking, dghRowHighlight]
      ReadOnly = True
      RowHeight = 4
      RowLines = 1
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      UseMultiTitle = True
      OnGetCellParams = dbgTTN17_newGetCellParams
      Columns = <
        item
          EditButtons = <>
          FieldName = 'SVED_NUM'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'NUM_CIST'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'NUM_KVIT'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'NOM_ZD'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
        end
        item
          EditButtons = <>
          FieldName = 'DATE_KVIT'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'SACCNUMB'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'SFACEACC'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'SAGENT'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'MOD_MODIF'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'VES'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'NPRICE'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'SUM_PROD'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'NDS_PROD'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'SUM_NACEN1'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'VOZNAGR'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'SUM_NACEN2'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
        end
        item
          EditButtons = <>
          FieldName = 'SUM_INSURE'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'TARIF'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end
        item
          EditButtons = <>
          FieldName = 'TARIF_GUARD'
          Footers = <>
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Title.TitleButton = True
        end>
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 84
    Width = 185
    Height = 595
    Align = alLeft
    TabOrder = 2
    object DBGridEh1: TDBGridEh
      Left = 1
      Top = 1
      Width = 183
      Height = 593
      Align = alClient
      AllowedOperations = []
      AutoFitColWidths = True
      DataSource = dsCat
      Flat = True
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -11
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = []
      Options = [dgColumnResize, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      OptionsEh = [dghFixed3D, dghHighlightFocus, dghRowHighlight, dghDialogFind]
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          EditButtons = <>
          FieldName = 'NAME'
          Footers = <>
          ImageList = frmMain.ImageList2
          ShowImageAndText = True
          Width = 206
          OnGetCellParams = DBGridEh1Columns0GetCellParams
        end>
    end
  end
  object ActionManager1: TActionManager
    Images = frmMain.ImageList1
    Left = 240
    Top = 264
    StyleName = 'XP Style'
    object acFilter: TAction
      Caption = #1054#1090#1086#1073#1088#1072#1090#1100
      ImageIndex = 0
      OnExecute = acFilterExecute
    end
    object acLoad: TAction
      Caption = #1047#1072#1075#1088#1091#1079#1080#1090#1100' '#1074' '#1055#1072#1088#1091#1089
      ImageIndex = 2
      OnExecute = acLoadExecute
    end
  end
  object OraQuery: TOracleQuery
    Session = DM.oraMain
    Left = 464
    Top = 153
  end
  object oraCatalog: TOracleDataSet
    SQL.Strings = (
      
        'select RN, CRN, NAME from v_acatalog where unitcode='#39'GoodsTransI' +
        'nvoicesToConsumers'#39' order by signs,crn nulls first ')
    QBEDefinition.QBEFieldDefs = {
      
      0B0000004841534348494C4452454E010000000000}
    ReadOnly = True
    Session = DM.oraParus
    Left = 348
    Top = 420
  end
  object oraQ: TOracleQuery
    Session = DM.oraParus
    Left = 437
    Top = 372
  end
  object oraTTN17_new: TOracleDataSet
    SQL.Strings = (
      
        'SELECT   k.sved_num, k.num_cist, k.date_cena as date_cen, k.num_' +
        'kvit,u.cat_cen_id,'
      
        '         k.date_kvit, p.saccdoc AS saccdoc, d.dog_number AS sacc' +
        'numb,p.valid_docnumb,'
      
        '         d.dog_date AS daccdate, p.sfaceacc, p.sagent, p.starif,' +
        ' p.sshipview,'
      
        '         p.spay_type, k.ves, p.nprice, p.proc_insure, m.gosprog_' +
        'id,m.nom_zd,'
      
        '         k.ves * 1000 * p.nprice AS sum_prod, p.usl_sum, k.flg_d' +
        'op_cist,'
      
        '         ROUND (k.ves * 1000 * p.nprice * 18/118, 2) AS nds_prod' +
        ','
      
        '         ROUND ((p.usl_sum),2) AS price_nacen1,ROUND ((k.flg_dop' +
        '_cist*35*1.18),2) AS price_nacen2,'
      
        '         k.sum_vozn11+k.sum_vozn11_nds AS sum_nacen1,ROUND (k.ve' +
        's*(k.flg_dop_cist*35*1.18),2) AS sum_nacen2,'
      '         Sum_vozn12+SUM_VOZN12_NDS AS voznagr,'
      
        '         ROUND (  ROUND (k.ves * 1000 * p.nprice,2)* p.proc_insu' +
        're/ 100,2) AS sum_insure,'
      
        '         k.tarif, k.tarif_nds, k.nom_zd AS srecipnumb,k.TARIF_GU' +
        'ARD, k.TARIF_GUARD_NDS,'
      '         TRIM (pr.gd_kod) || '#39'/'#39' || TRIM (pr.okpo) AS sagnfifo,'
      '         k.date_kvit AS drptdate, p.sacc_agent, p.ssubdiv, '
      '         stan.stan_name, pn.modif, pn.mod_modif, '
      
        '         p.sjur_pers, k.ID,vo.vagown_typ_id,d.id as dogid,pn.tax' +
        'gr_code,pn.taxgr_code_new,k.no_akciz'
      '    FROM kvit k, MONTH m, zakaz z, zakaz unp,'
      '         kls_dog d,usl_dog u,'
      '         v_parus_price p,'
      '         kls_predpr pr,'
      '         kls_stan stan,'
      '         kls_vid_otgr v,'
      '         kls_prod_nomenklator pn,'
      #9'  kls_vagowner vo'
      '   WHERE k.nom_zd = m.nom_zd'
      '     AND m.nom_zd = TRIM (p.ord_pref)'
      
        '     AND z.dog_id = d.ID and (m.dog_id=u.DOG_ID and m.usl_number' +
        '=u.USL_NUMBER)'
      '     AND pn.prod = k.prod_id_npr'
      '     AND m.poluch_id = pr.ID'
      '     AND stan.ID = m.stan_id and k.vagowner_id=vo.id'
      '     AND m.load_abbr = v.load_abbr'
      #9' and m.ZAKAZ_ID=unp.ID and unp.IS_AGENT=1'
      
        '     AND z.IS_AGENT=2 and unp.LINK_ID=z.ID and d.PREDPR_ID<>2641' +
        ' '
      #9' and pn.is_actual=1'
      '     AND k.bill_id IS NOT NULL AND k.bill_id<>0'
      '     AND m.nazn_otg_id <> 2'
      '     AND (v.load_type_id = 1 OR v.load_type_id = 6)'
      
        '     and not exists (select kvit_id from parus_fin where k.id = ' +
        'parus_fin.kvit_id and task = '#39'GoodsTransInvoicesToConsumers'#39'  an' +
        'd subtask<>2)'
      #9#9#9#9#9'   '
      #9#9#9#9#9'   '
      ''
      ''
      #9#9#9#9#9'   '
      'ORDER BY k.sved_num')
    QBEDefinition.QBEFieldDefs = {
      
      4557010000000000080000004E4F5F414B43495A010000000000}
    ReadOnly = True
    Session = DM.oraMain
    Left = 544
    Top = 152
    object oraTTN17_newSVED_NUM: TIntegerField
      DisplayLabel = #1057#1074#1077#1076#1077#1085#1080#1077
      FieldName = 'SVED_NUM'
    end
    object oraTTN17_newNUM_CIST: TStringField
      DisplayLabel = #1062#1080#1089#1090#1077#1088#1085#1072
      FieldName = 'NUM_CIST'
      Size = 10
    end
    object oraTTN17_newCAT_CEN_ID: TIntegerField
      FieldName = 'CAT_CEN_ID'
      Required = True
    end
    object oraTTN17_newDATE_CEN: TDateTimeField
      FieldName = 'DATE_CEN'
    end
    object oraTTN17_newNUM_KVIT: TFloatField
      DisplayLabel = #1050#1074#1080#1090#1072#1085#1094#1080#1103
      FieldName = 'NUM_KVIT'
      Required = True
    end
    object oraTTN17_newDATE_KVIT: TDateTimeField
      DisplayLabel = #1044#1072#1090#1072
      FieldName = 'DATE_KVIT'
    end
    object oraTTN17_newSACCDOC: TStringField
      FieldName = 'SACCDOC'
      Required = True
      Size = 10
    end
    object oraTTN17_newSACCNUMB: TStringField
      DisplayLabel = #1044#1086#1075#1086#1074#1086#1088
      FieldName = 'SACCNUMB'
      Required = True
      Size = 30
    end
    object oraTTN17_newDACCDATE: TDateTimeField
      FieldName = 'DACCDATE'
      Required = True
    end
    object oraTTN17_newSFACEACC: TStringField
      DisplayLabel = #1051#1057
      FieldName = 'SFACEACC'
    end
    object oraTTN17_newSAGENT: TStringField
      DisplayLabel = #1050#1086#1085#1090#1088#1072#1075#1077#1085#1090
      FieldName = 'SAGENT'
      Required = True
    end
    object oraTTN17_newSTARIF: TStringField
      FieldName = 'STARIF'
    end
    object oraTTN17_newSSHIPVIEW: TStringField
      FieldName = 'SSHIPVIEW'
    end
    object oraTTN17_newSPAY_TYPE: TStringField
      FieldName = 'SPAY_TYPE'
    end
    object oraTTN17_newVES: TFloatField
      DisplayLabel = #1042#1077#1089
      FieldName = 'VES'
      Required = True
    end
    object oraTTN17_newNPRICE: TFloatField
      DisplayLabel = #1062#1077#1085#1072
      FieldName = 'NPRICE'
    end
    object oraTTN17_newPROC_INSURE: TFloatField
      FieldName = 'PROC_INSURE'
    end
    object oraTTN17_newGOSPROG_ID: TIntegerField
      FieldName = 'GOSPROG_ID'
      Required = True
    end
    object oraTTN17_newSUM_PROD: TFloatField
      DisplayLabel = #1055#1088#1086#1076#1091#1082#1090'|'#1057#1091#1084#1084#1072
      FieldName = 'SUM_PROD'
    end
    object oraTTN17_newUSL_SUM: TFloatField
      FieldName = 'USL_SUM'
      Required = True
    end
    object oraTTN17_newFLG_DOP_CIST: TIntegerField
      FieldName = 'FLG_DOP_CIST'
      Required = True
    end
    object oraTTN17_newNDS_PROD: TFloatField
      DisplayLabel = #1055#1088#1086#1076#1091#1082#1090'|'#1053#1044#1057
      FieldName = 'NDS_PROD'
    end
    object oraTTN17_newVOZNAGR: TFloatField
      DisplayLabel = #1042#1086#1079#1085#1072#1075#1088#1072#1078#1076#1077#1085#1080#1077'|'#1057#1091#1084#1084#1072'2'
      FieldName = 'VOZNAGR'
    end
    object oraTTN17_newSUM_INSURE: TFloatField
      DisplayLabel = #1057#1090#1088#1072#1093#1086#1074#1082#1072
      FieldName = 'SUM_INSURE'
    end
    object oraTTN17_newTARIF: TFloatField
      DisplayLabel = #1058#1072#1088#1080#1092'|'#1057#1091#1084#1084#1072
      FieldName = 'TARIF'
      Required = True
    end
    object oraTTN17_newTARIF_NDS: TFloatField
      DisplayLabel = #1058#1072#1088#1080#1092'|'#1053#1044#1057
      FieldName = 'TARIF_NDS'
      Required = True
    end
    object oraTTN17_newSRECIPNUMB: TStringField
      FieldName = 'SRECIPNUMB'
      Required = True
      Size = 12
    end
    object oraTTN17_newSAGNFIFO: TStringField
      FieldName = 'SAGNFIFO'
      Size = 23
    end
    object oraTTN17_newDRPTDATE: TDateTimeField
      FieldName = 'DRPTDATE'
    end
    object oraTTN17_newSACC_AGENT: TStringField
      FieldName = 'SACC_AGENT'
    end
    object oraTTN17_newSSUBDIV: TStringField
      FieldName = 'SSUBDIV'
    end
    object oraTTN17_newSJUR_PERS: TStringField
      FieldName = 'SJUR_PERS'
      Required = True
    end
    object oraTTN17_newID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object oraTTN17_newSTAN_NAME: TStringField
      FieldName = 'STAN_NAME'
      Required = True
      Size = 50
    end
    object oraTTN17_newMODIF: TStringField
      FieldName = 'MODIF'
      Required = True
    end
    object oraTTN17_newMOD_MODIF: TStringField
      DisplayLabel = #1055#1088#1086#1076#1091#1082#1090
      FieldName = 'MOD_MODIF'
    end
    object oraTTN17_newTARIF_GUARD: TFloatField
      DisplayLabel = #1058#1072#1088#1080#1092'|'#1054#1093#1088#1072#1085#1072
      FieldName = 'TARIF_GUARD'
      Required = True
    end
    object oraTTN17_newTARIF_GUARD_NDS: TFloatField
      FieldName = 'TARIF_GUARD_NDS'
      Required = True
    end
    object oraTTN17_newVALID_DOCNUMB: TStringField
      FieldName = 'VALID_DOCNUMB'
    end
    object oraTTN17_newNOM_ZD: TStringField
      DisplayLabel = #1047#1072#1103#1074#1082#1072
      FieldName = 'NOM_ZD'
      Required = True
      Size = 12
    end
    object oraTTN17_newVAGOWN_TYP_ID: TFloatField
      FieldName = 'VAGOWN_TYP_ID'
      Required = True
    end
    object oraTTN17_newDOGID: TIntegerField
      FieldName = 'DOGID'
      Required = True
    end
    object oraTTN17_newPRICE_NACEN1: TFloatField
      FieldName = 'PRICE_NACEN1'
    end
    object oraTTN17_newPRICE_NACEN2: TFloatField
      FieldName = 'PRICE_NACEN2'
    end
    object oraTTN17_newSUM_NACEN1: TFloatField
      DisplayLabel = #1042#1086#1079#1085#1072#1075#1088#1072#1078#1076#1077#1085#1080#1077'|'#1057#1091#1084#1084#1072'1'
      FieldName = 'SUM_NACEN1'
    end
    object oraTTN17_newSUM_NACEN2: TFloatField
      DisplayLabel = #1042#1086#1079#1085#1072#1075#1088#1072#1078#1076#1077#1085#1080#1077'|'#1057#1091#1084#1084#1072'3'
      FieldName = 'SUM_NACEN2'
    end
    object oraTTN17_newTAXGR_CODE: TStringField
      FieldName = 'TAXGR_CODE'
    end
    object oraTTN17_newTAXGR_CODE_NEW: TStringField
      FieldName = 'TAXGR_CODE_NEW'
    end
    object oraTTN17_newNO_AKCIZ: TIntegerField
      FieldName = 'NO_AKCIZ'
    end
  end
  object dsTTN17_new: TDataSource
    DataSet = oraTTN17_new
    Left = 544
    Top = 200
  end
  object oraInvIns: TOracleQuery
    SQL.Strings = (
      'begin'
      'P_TRANSINVCUST_INSERT'
      '('
      ' :nCOMPANY,'
      ' :nCRN,'
      ' :sJUR_PERS,'
      ' :sDOCTYPE,'
      ' :sPREF,'
      ' :sNUMB,'
      ' :dDOCDATE,'
      ' :dSALEDATE,'
      ' :sACCDOC,'
      ' :sACCNUMB,'
      ' :dACCDATE,'
      ' :sDIRDOC,'
      ' :sDIRNUMB,'
      ' :dDIRDATE,'
      ' :sSTOPER,'
      ' :sFACEACC,'
      ' :sGRAPHPOINT,'
      ' :sAGENT,'
      ' :sTARIF,'
      ' :nSERVACT_SIGN,'
      ' :sSTORE,'
      ' :sMOL,'
      ' :sSHEEPVIEW,'
      ' :sPAYTYPE,'
      ' :nDISCOUNT,'
      ' :sCURRENCY,'
      ' :nCURCOURS,'
      ' :nCURBASE,'
      ' :nFA_COURS,'
      ' :nFA_BASECOURS,'
      ' :nSUMM,'
      ' :nSUMMWITHNDS,'
      ' :sRECIPDOC,'
      ' :sRECIPNUMB,'
      ' :dRECIPDATE,'
      ' :sFERRYMAN,'
      ' :sAGNFIFO,'
      ' :sFORWARDER,'
      ' :sWAYBLADENUMB,'
      ' :sDRIVER,'
      ' :sCAR,'
      ' :sROUTE,'
      ' :sTRAILER1,'
      ' :sTRAILER2,'
      ' :sCOMMENTS,'
      ' :sACC_AGENT,'
      ' :sSUBDIV,'
      ' :nRN,'
      ' :sBARCODE,'
      ' :sMSG'
      ');'
      'end;')
    Session = DM.oraParus
    Variables.Data = {
      
      00000000000000000000}
    Left = 396
    Top = 329
  end
  object oraInvsIns: TOracleQuery
    SQL.Strings = (
      'begin'
      'P_TRANSINVCUSTSPECS_INSERT'
      '('
      '  :nCOMPANY,'
      '  :nPRN,'
      '  :sTAXGR,'
      '  :sGOODSPARTY,'
      '  :sNOMEN,'
      '  :sNOMNMODIFPACK,'
      '  :sNOMMODIF,'
      '  :sARTICLE,'
      '  :sCELL,'
      '  :nPRICE,'
      '  :nDISCOUNT,'
      '  :nQUANT,'
      '  :nQUANTALT,'
      '  :nPRICEMEAS,'
      '  :nSUMM,'
      '  :nSUMMWITHNDS,'
      '  :dBEGINDATE,'
      '  :dENDDATE,'
      '  :sSERNUMB,'
      '  :sCOUNTRY,'
      '  :sGTD,'
      '  :sNOTE,'
      '  :nRN,'
      '  :sMSG'
      ');'
      'end;')
    Session = DM.oraParus
    Variables.Data = {
      
      003A534D5347050000000000000000000000}
    Left = 444
    Top = 329
  end
  object qLinkIn: TOracleQuery
    SQL.Strings = (
      'begin'
      'P_DOCINPT_BASE_INSERT'
      '('
      '  :nCOMPANY,'
      '  :sUNITCODE,'
      '  :nDOCUMENT,'
      '  :nPRN_DOCUMENT,'
      '  :dIN_DATE,'
      '  :nSTATUS,'
      '  :nBREAKUP_KIND,'
      '  :nRN'
      ');'
      'end;')
    Session = DM.oraParus
    Variables.Data = {
      
      0000040000003A4E524E030000000000000000000000}
    Left = 396
    Top = 414
  end
  object qLink: TOracleQuery
    SQL.Strings = (
      'begin'
      'P_DOCLINKS_INSERT'
      '('
      '  :nIN_DOC,'
      '  :nOUT_DOC'
      ');'
      'end;')
    Session = DM.oraParus
    Variables.Data = {
      0300000002000000080000003A4E494E5F444F43030000000000000000000000
      090000003A4E4F55545F444F43030000000000000000000000}
    Left = 436
    Top = 413
  end
  object qLinkOut: TOracleQuery
    SQL.Strings = (
      'begin'
      'P_DOCOUTPT_BASE_INSERT'
      '('
      '  :nCOMPANY,'
      '  :sUNITCODE,'
      '  :nDOCUMENT,'
      '  :nPRN_DOCUMENT,'
      '  :dIN_DATE,'
      '  :nSTATUS,'
      '  :nBREAKUP_KIND,'
      '  :nRN'
      ');'
      'end;'
      '')
    Session = DM.oraParus
    Variables.Data = {
      
      0000040000003A4E524E030000000000000000000000}
    Left = 476
    Top = 414
  end
  object qInS: TOracleQuery
    SQL.Strings = (
      'begin'
      'P_DOCINPT_FIND('
      '  1,'
      '  2,'
      '  :sunitcode,'
      '  :nDOCUMENT,'
      '  :nRESULT,'
      '  1);'
      'end;')
    Session = DM.oraParus
    Variables.Data = {
      03000000030000000A0000003A4E444F43554D454E5403000000000000000000
      0000080000003A4E524553554C540300000000000000000000000A0000003A53
      554E4954434F4445050000000000000000000000}
    Left = 396
    Top = 371
  end
  object dsCat: TDataSource
    DataSet = mteCatalog
    Left = 352
    Top = 472
  end
  object mteCatalog: TMemTableEh
    FieldDefs = <>
    FetchAllOnOpen = True
    IndexDefs = <
      item
        Name = 'mteCatalogIndex1'
        Fields = 'rn'
      end>
    Params = <>
    DataDriver = dsdCatalog
    StoreDefs = True
    TreeList.Active = True
    TreeList.KeyFieldName = 'rn'
    TreeList.RefParentFieldName = 'crn'
    Left = 400
    Top = 472
    object mteCatalogRN: TFloatField
      FieldName = 'RN'
      Required = True
    end
    object mteCatalogCRN: TFloatField
      FieldName = 'CRN'
    end
    object mteCatalogNAME: TStringField
      DisplayLabel = #1050#1072#1090#1072#1083#1086#1075
      FieldName = 'NAME'
      Required = True
      Size = 160
    end
  end
  object dsdCatalog: TDataSetDriverEh
    KeyFields = 'rn'
    ProviderDataSet = oraCatalog
    ResolveToDataSet = False
    Left = 472
    Top = 472
  end
end

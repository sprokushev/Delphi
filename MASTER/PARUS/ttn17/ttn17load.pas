...
Procedure TfrmTTN17.WriteEvent(Tag: String);
Begin
frmMain.WriteEvent(Tag, ' Форма "Расходные накладные потребителям в Парус"');
End;
...
Begin
  ...
  While Not ss^.Eof Do
    Begin
      WriteEvent('Закачка в Парус сведение №' + ss^.fieldbyname('sved_num').AsString + ' №' + ss^.fieldbyname('num_cist').AsString);
      ok := false;
      // Партия
...
//          summ:='0';
          ...
          if (ss^.fieldbyname('date_kvit').asDateTime >= StrToDate('01.01.2006')) and (ss^.fieldbyname('date_kvit').asDateTime < StrToDate('01.01.2007')) and (ss^.fieldbyname('no_akciz').AsInteger=1) then
              ...
              begin
                oraInvsIns.Execute;
                WriteEvent('Спецификация продукта загружена...');
              end;
              ...
              If ss^.fieldbyname('sum_insure').asFloat > 0 Then
                Begin
                  ...
                  begin
                    oraInvsIns.Execute;
                    WriteEvent('СТраховка загружена...');
                  end;
                  Except On E: EOracleError Do
                      ShowMessage(E.Message);
                  End; //свед 3
// Вознаграждение за срочность
               If ss^.fieldbyname('sum_nacen2').asFloat> 0 Then
                Begin
                  ...
                  begin
                    oraInvsIns.Execute;
                    WriteEvent('Вознаграждение за срочность загружено...');
                  end;
                  Except On E: EOracleError Do
                      ShowMessage(E.Message);
                  End; //свед 4
...
//Вознаграждение
              If ss^.fieldbyname('sum_nacen1').asFloat> 0 Then
               Begin
                  ...
                  begin
                    oraInvsIns.Execute;
                    WriteEvent('Вознаграждение загружено...');
                  end;
                  Except On E: EOracleError Do
                      ShowMessage(E.Message);
                  End; //свед 4
...
//Аренда
              If ss^.fieldbyname('voznagr').asFloat > 0 Then
               Begin
                  ...
                  begin
                    oraInvsIns.Execute;
                    WriteEvent('Вознаграждение за аренду загружено...');
                  end;
                  Except On E: EOracleError Do
                      ShowMessage(E.Message);
                  End; //свед 4
  ...
  While Not ss^.Eof Do
    Begin
      ...
      Else
        Begin
          ok := false;
          WriteEvent('Не найдена станция в Парусе: ' + st1);
          WriteEvent('Закачка в Парус сведение №' + ss^.fieldbyname('sved_num').AsString + 'не произведена!');
        End;
      oraQ.SQL.Clear;
      If ok Then
        Begin
          ...
          If ss^.fieldbyname('tarif').asFloat > 0 Then
            Begin
              WriteEvent('Загрузка акта транспортировки...');
              oraInvIns.ClearVariables;
              oraInvIns.SetVariable(':nCOMPANY', 2);
              oraInvIns.SetVariable(':nCRN', rn_crn);
              oraInvIns.SetVariable(':sJUR_PERS', trim(ss^.fieldbyname('sjur_pers').asString));

...
uses
  ...
  Dialogs,DateUtils, StdCtrls, Mask, DBCtrlsEh, Grids, CustomDBGridEhImpExp, DBGridEh,
  ActnList, DB, XPStyleActnCtrls, ActnMan, OracleData, ToolWin, ActnCtrls,
  ExtCtrls, Oracle, ComCtrls, RxShell, MDIForm, GridsEh;
...
type
  Tf_Export2R3 = class(Tf_MDIForm)
    ...
    procedure acDelRaznarExecute(Sender: TObject);
  private
...
begin
  case cbUpload.ItemIndex of
    ...
    3:begin
        ...
        if edNumCist.Text<>'' then
        Begin
          qNewKvit.SQL[52]:='and kvit.NUM_CIST LIKE '''+edNumCist.Text+'%''';
        End
        else
        Begin
          qNewKvit.SQL[52]:='and kvit.NUM_CIST LIKE ''%''';
        End;
    ...
    4:begin
        ...
        if edNumCist.Text<>'' then
          qPasp.SQL[25]:='and kvit.NUM_CIST LIKE '''+edNumCist.Text+'%'''
        else
          qPasp.SQL[25]:='and kvit.NUM_CIST LIKE ''%''';
...
begin
  if Application.MessageBox(PChar('Очистить № поставки SAP R/3 '+qDeblok.FieldByName('ID_SAP').AsString+' у вагона '+qDeblok.FieldByName('TRAID').AsString+' ?'),'Внимание!',MB_YESNO)=IDYES then
  Begin
    try
      pkgFOR_R3.CallProcedure('DelRCRow',[1,qDeblok.FieldByName('ID_SAP').AsString]);
    except
      on E: Exception do
      Begin
        // Отобразим ошибку
...
procedure Tf_Export2R3.acDelRaznarExecute(Sender: TObject);
begin
  if cbUpload.ItemIndex<>2 then
  begin
    Application.MessageBox(PChar('Выберите тип отгрузки СУТОЧНЫЕ РАЗНАРЯДКИ!'),'Ошибка!');
    Exit;
  end;
  if Application.MessageBox(PChar('Очистить № разнарядки SAP R/3 '+qSutRazn.FieldByName('R3_RAZNAR_NUM').AsString+'?'),'Внимание!',MB_YESNO)=IDYES then
    try
      pkgFOR_R3.CallProcedure('DelVbakRow',[1,qSutRazn.FieldByName('R3_RAZNAR_NUM').AsString]);
    except
      on E: Exception do
      Begin
        // Отобразим ошибку

...
type
  Tf_Zakaz = class(TForm)
    ...
    q_AgentPERIOD_NAME: TStringField;
    q_AgentPERIOD_ID: TFloatField;
    q_PostavTIP_CORP_NAME: TStringField;
    q_AgentTIP_CORP_NAME: TStringField;
    q_PostavPERIOD_ID: TFloatField;
    q_PostavTIP_CORP_ID: TFloatField;
    q_AgentTIP_CORP_ID: TFloatField;
    q_PostavHistKOD_ISU: TStringField;
    q_AgentHistKOD_ISU: TStringField;
    acZakazHistWord2: TAction;
    TBItem101: TTBItem;
    q_PostavDOG_DATE: TDateTimeField;
    q_PostavPLAT_REG: TStringField;
    q_PostavGDOR_NAME: TStringField;
    q_PostavSTAN_REG: TStringField;
    q_PostavPRICE_NDS: TFloatField;
    q_PostavPRICE_WITH_NDS: TFloatField;
    acRequestConsumerToSAP: TAction;
    TBItem102: TTBItem;
    ...
    procedure acZakazHistWord2Execute(Sender: TObject);
    procedure acRequestConsumerToSAPExecute(Sender: TObject);
  private
    ...
    procedure SaveToWord2(q_Source:TOracleDataSet;q_SourceHist:TOracleDataSet);
...
uses ForDB, main, ZakazNew, ZakazHistNew, ForFiles, ComObj, OleServer, ExcelXP,
  OraToDbf, oper_zayav, ForSystem, Export2Pererabotka;
...
begin
  ...
  Begin
    ...
    pkgFOR_ZAKAZ.CallProcedure('FillTemp',[2,DateOf(edDateFrom.Date),DateOf(edDateTo.Date),q_PostavID.AsFloat]);
...
begin
  ...
  Begin
    ...
    // Обновить данные во временной таблице
    pkgFOR_ZAKAZ.CallProcedure('FillTemp',[1,DateOf(edDateFrom.Date),DateOf(edDateTo.Date),q_AgentID.Asfloat]);
...
begin
  ...
  Begin
    ...
    if (PageControlZakaz.ActivePage=tabAgent) then
    Begin
      EditId:=q_AgentID.Asfloat;
      vIS_AGENT:=1;
      vAGENT_ID:=AgentID;
    end;
    if (PageControlZakaz.ActivePage=tabPostav) then
    Begin
      EditId:=q_PostavID.Asfloat;
      vIS_AGENT:=2;
      vAGENT_ID:=AgentID;
    end;
    showmodal;
    ...
    Begin
      if q_PostavIS_ACCEPT.AsInteger=0 then
        ...
        Begin
          try
            q_Postav.DisableControls;
            pkgFOR_ZAKAZ.CallProcedure('DelZakaz',[1,q_PostavID.Asfloat,0]);
            IsRefresh:=true;
            q_Postav.Next;
          except
    ...
    Begin
      if q_AgentIS_ACCEPT.AsInteger=0 then
        ...
        Begin
          try
            q_Agent.DisableControls;
            pkgFOR_ZAKAZ.CallProcedure('DelZakaz',[1,q_AgentID.Asfloat,0]);
            IsRefresh:=true;
            q_Agent.Next;
          except
            on E:EOracleError do
...
begin
  ...
  Begin
    ...
    Begin
      ...
      EditId:=q_AgentID.Asfloat;
      EditHistId:=q_AgentHistID.Asfloat;
      vIS_AGENT:=1;
      vAGENT_ID:=AgentID;
    ...
    Begin
      ...
      EditId:=q_PostavID.Asfloat;
      EditHistId:=q_PostavHistID.Asfloat;
      vIS_AGENT:=2;
      vAGENT_ID:=AgentID;
  ...
  Begin
    ...
    Begin
      ...
      try
        ...
        pkgFOR_ZAKAZ.CallProcedure('DelZakazHist',[1,q_PostavHistID.Asfloat,0]);
        q_PostavHist.Next;
      except
        on E:EOracleError do
          ShowMessage(E.Message);
      end;
      NextID:=q_PostavHistID.Asfloat;
      IsRefresh:=true;
      f_db.ReQuery(q_Postav,true);
      IsRefresh:=false;
      q_PostavAfterScroll(q_Postav);
    ...
    Begin
      ...
      try
        ...
        pkgFOR_ZAKAZ.CallProcedure('DelZakazHist',[1,q_AgentHistID.AsFloat,0]);
        q_AgentHist.Next;
      except
        on E:EOracleError do
          ShowMessage(E.Message);
      end;
      NextID:=q_AgentHistID.Asfloat;
      IsRefresh:=true;
      f_db.ReQuery(q_Agent,true);
      IsRefresh:=false;
      q_AgentAfterScroll(q_Agent);
...
procedure Tf_Zakaz.SaveToWord2(q_Source:TOracleDataSet;q_SourceHist:TOracleDataSet);
const
  wdGoToBookmark=$FFFFFFFF;
var
  MSWord   :Variant;
...
begin
  inherited;
  f_fileutils.SafeCopyFile('u:\master\doc\specif.doc', f_main.TempLocalPath+'specif.doc');
  MSWord := createOleObject('Word.Application');
  try
    MSWord.application.documents.add(template := f_main.TempLocalPath+'specif.doc');
  except
  end;
  MSWord.application.visible := true;
  MSWord.application.activate;
...
{  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='To');
  ...
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Cena_otp_bn');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('price').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Cena_otp_nds');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('price_nds').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Cena_otp');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('price_with_nds').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='ClientNum');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('client_number').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='ClientDate');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('client_date').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Dogovor');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('dog_number').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='DogNum');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('dog_number').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='DogDate');
  MSWord.application.Selection.TypeText(FormatDateTime('dd.mm.yyyy', q_Source.FieldByName('dog_date').asDateTime));
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='DogDate2');
  MSWord.application.Selection.TypeText(FormatDateTime('dd.mm.yyyy', q_Source.FieldByName('dog_date').asDateTime));

  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Npr');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('name_npr').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='PlanMon');
  MSWord.application.Selection.TypeText(FormatDateTime('mmmm yyyy', q_Source.FieldByName('date_plan').asDateTime));
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Plat');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('plat_name').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='RegPlat');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('plat_reg').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='RegStan');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('stan_reg').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='RGD');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('gdor_name').AsString);

  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='SpecifNum');
  MSWord.application.Selection.TypeText('00001');
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='SpecifDate');
  MSWord.application.Selection.TypeText(FormatDateTime('dd.mm.yyyy', Now));
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Stan');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('Stan_name').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Ves');
  MSWord.application.Selection.TypeText(q_SourceHist.FieldByName('load_ves').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Summa_Otp');
  MSWord.application.Selection.TypeText(F_ForSystem.RealToStr(q_SourceHist.FieldByName('load_ves').AsFloat*q_Source.FieldByName('price_with_nds').AsFloat));
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='Vetka');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('VETKA_NAME').AsString);
  MSWord.application.Selection.GoTo(What:=wdGoToBookmark, Name:='usl_opl');
  MSWord.application.Selection.TypeText(q_Source.FieldByName('USL_OPL_NAME').AsString);

  str:=f_main.TempLocalPath+'s'+q_SourceHist.FieldByName('CLIENT_NUMBER').AsString+'_'+q_SourceHist.FieldByName('INPUT_NUMBER').AsString+'_'+trim(q_Source.FieldByName('id').AsString)+'.doc';
  try
    MSWord.application.ActiveDocument.SaveAs(str);
  except
  end;
...
begin
  ...
  if (PageControlZakaz.ActivePage=tabAgent) then
    ...
    Begin
      ...
      pkgFOR_ZAKAZ.CallProcedure('RefreshZakazHist',[1,q_PostavHistID.Asfloat,0]);
...
procedure Tf_Zakaz.acZakazHistWord2Execute(Sender: TObject);
begin
  if (PageControlZakaz.ActivePage=tabAgent) AND ((q_AgentHist.EOF) or (q_AgentHistSTATUS_ZAKAZ_ID.AsInteger<>20)) then exit;
  if (PageControlZakaz.ActivePage=tabPostav) AND ((q_PostavHist.EOF) or (q_PostavHistSTATUS_ZAKAZ_ID.AsInteger<>20)) then exit;
  if q_Postav.State=dsEdit then q_Postav.Post;
  ...
  if (PageControlZakaz.ActivePage=tabAgent) then
    ...
    Begin
      // Инициируем сохранение позиции "К отгрузке" - для верности, чтобы появился заказ в Заказах УНП
      pkgFOR_ZAKAZ.CallProcedure('RefreshZakazHist',[1,q_PostavHistID.Asfloat,0]);
      SaveToWord2(q_Postav,q_PostavHist);
    end;
end;

procedure Tf_Zakaz.acRequestConsumerToSAPExecute(Sender: TObject);
begin
  if q_Postav.State=dsEdit then q_Postav.Post;
  if q_Agent.State=dsEdit then q_Agent.Post;
  if q_PostavHist.State=dsEdit then q_PostavHist.Post;
  ...
  Begin
    ...
    acImportRaznar.Visible:=false;
    acImportXLS.Visible:=false;
    acDel_SAP.Visible:=false;
    acShowSQL.Visible:=false;
    rgNapr_post.Enabled:=false;
    GroupBox2.Enabled:=false;

    edBegin.Value:=StartOfTheMonth(edDateFrom.DateTime); // дата плана
    edEnd.Value:=StartOfTheMonth(edBegin.Value-1); // дата регистрации заявки
    cbUpload.ItemIndex:=6;

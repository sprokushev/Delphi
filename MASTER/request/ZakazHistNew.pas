...
uses
  ...
  Placemnt, RXCtrls, DBCtrls, ExtCtrls, Grids, DBGridEh, DBGrids, GridsEh;
...
type
  ...
  private
    ...
    LINK_HIST_ID:double;
  public
    EditID:double; // ID заказа
    EditHistID:double; // ID позиции
    ParentDover:TForm;
    { Public declarations }
       ...
       try
         if vADD=1 then
           EditHistID:=pkgFOR_ZAKAZ.CallFloatFunction('AddZakazHist',[0,0,0,0,EditID,vSTATUS_ZAKAZ,edClientNum.Text,
                  ...
                  '',0,0,0,0,0,DateOf(edBeginDate.Date),LINK_ID,LINK_HIST_ID,edISU.Text])
         else
       ...
       try
         if vADD=1 then
           ...
           EditHistID:=pkgFOR_ZAKAZ.CallFloatFunction('AddZakazHist',[0,EditHistId,0,vSORTBY,EditID,vSTATUS_ZAKAZ,edClientNum.Text,
                  ...
                  vNOM_ZD,edVes.Value,edKol.Value,edFactVes.Value,edFactKol.Value,cbEtran.KeyValue,DateOf(edBeginDate.Date),LINK_ID,LINK_HIST_ID,edISU.Text]);
...
begin
  ...
  Begin
    LINK_ID:=q_SourceHist.FieldByName('LINK_ID').Asfloat;
    LINK_HIST_ID:=q_SourceHist.FieldByName('LINK_HIST_ID').Asfloat;
    ...
    edISU.Text:=q_SourceHist.FieldByName('KOD_ISU').AsString;
  end
  else

...
type
  Tf_f42 = class(TForm)
    ...
    q_pryam_sf_ora: TOracleDataSet;
    q_fact_plat_ora: TOracleDataSet;
    q_sev_np_sf_ora: TOracleDataSet;
    ...
    procedure q_fact_plat_oraBeforeOpen(DataSet: TDataSet);
  private
...
begin
  ...
  begin
    q_fact_ora.Open;
    q_grplp.Open;
    while not q_fact_ora.Eof do
      begin
        pbar.Position:=Round(q_fact_ora.RecNo/q_fact_ora.RecordCount*100/5);
        pbar.Refresh;
        if True then
          begin
            if q_grplp.Locate('kod_sgr;kod_spg;kod_rzd;kod_prz;kod_grp',VarArrayOf([q_fact_ora.FieldByName('KOD_SGR').AsInteger,q_fact_ora.FieldByName('KOD_SPG').AsInteger,q_fact_ora.FieldByName('KOD_RZD').AsInteger,q_fact_ora.FieldByName('KOD_PRZ').AsInteger,q_fact_ora.FieldByName('KOD_GRP').AsInteger]),[]) then
              begin
                gr_name:=Trim(q_grplp.FieldByName('naim').AsString);
                gr_pos:=q_grplp.FieldByName('kod_pos').AsString
              end
              ...
              begin
                gr_name:=Trim(q_fact_ora.FieldByName('Naim').AsString);
                gr_pos:=q_Fact_ora.FieldByName('kod_pos').AsString;
              end;
            q_cmn2.SQl.Clear;
            q_cmn2.SQL.Add('INSERT INTO "'+f_main.TempLocalPath+'F42_PLN1.DBF" (kod_npr,kod_pos,kod_sgr,kod_spg,kod_rzd,kod_prz,kod_grp,naim,d_ves,v_ves,d_sum,v_sum,od_ves,ov_ves,od_sum,ov_sum,Fd_ves,Fv_ves,Fd_sum,Fv_sum,zayv_tonn,zayv_dat) VALUES (');
            q_cmn2.SQL.ADD(''''+q_fact_ora.FieldByName('KOD_NPR').AsString+''',');
            q_cmn2.SQL.Add(gr_pos+',');
            q_cmn2.SQL.Add(q_Fact_ora.FieldByName('kod_sgr').AsString+','+q_Fact_ora.FieldByName('kod_spg').AsString+',');
            q_cmn2.SQL.Add(q_Fact_ora.FieldByName('kod_rzd').AsString+','+q_Fact_ora.FieldByName('kod_prz').AsString+',');
            q_cmn2.SQL.Add(q_Fact_ora.FieldByName('kod_grp').AsString+','''+gr_name+''',0,0,0,0,0,0,0,0,');
            q_cmn2.SQL.Add(FloatToStr(q_Fact_ora.FieldByName('d_ves').AsFloat)+','+FloatToStr(q_Fact_ora.FieldByName('v_ves').AsFloat)+',');
            q_cmn2.SQL.Add(FloatToStr(q_Fact_ora.FieldByName('d_sum').AsFloat)+','+FloatToStr(q_Fact_ora.FieldByName('v_sum').AsFloat)+',0,{01/01/1990})');
            q_cmn2.ExecSQL;
          end;
        q_fact_ora.Next;
      end;
...
begin
  ...
  q_fact_plat_ora.SQL.SaveToFile('c:\1.sql');
end;
...
begin
  ...
  if rg_TypeSelect.ItemIndex=0 then
    ...
    begin
      q_pryam_sf_ora.SetVariable(':BEG_DATE',date1.date);
      q_pryam_sf_ora.SetVariable(':END_DATE',date2.date);
      q_pryam_sf_ora.Open;
      while not q_pryam_sf_ora.eof do
        begin
          q_cmn.SQL.Clear;
          q_cmn.SQL.Add('INSERT INTO "'+f_main.TempLocalPath+'optov" (kod_plat,kod_npr,ves,sum) VALUES ('+
            q_pryam_sf_ora.FieldByName('kod_plat').AsString+','+
            ''''+q_pryam_sf_ora.FieldByName('kod_npr').AsString+''','+
            FloatToStr(q_pryam_sf_ora.FieldByName('ves').AsFloat/1000)+','+
            FloatToStr(q_pryam_sf_ora.FieldByName('sum').AsFloat/1000)+
          ')');
          q_cmn.ExecSQL;
          q_pryam_sf_ora.next;
        end;
      q_pryam_sf_ora.close;
    end;
  q_cmn.Close;
end;
...
begin
  ...
  if rg_TypeSelect.ItemIndex=0 then
    ...
    begin
      with q_sev_np_sf_ora do
      begin
        SetVariable(':BEG_DATE',date1.date);
        SetVariable(':END_DATE',date2.date);
        Open;
        while not EOF do
        begin
          if q_grplp.Locate('kod_sgr;kod_spg;kod_rzd;kod_prz;kod_grp',VarArrayOf([FieldByName('KOD_SGR').AsInteger,FieldByName('KOD_SPG').AsInteger,FieldByName('KOD_RZD').AsInteger,FieldByName('KOD_PRZ').AsInteger,FieldByName('KOD_GRP').AsInteger]),[]) then
            begin
              gr_name:=Trim(q_grplp.FieldByName('naim').AsString);
              gr_pos:=q_grplp.FieldByName('kod_pos').AsString
            end
            ...
            begin
              gr_name:=Trim(FieldByName('Naim').AsString);
              gr_pos:=FieldByName('kod_pos').AsString;
            end;
            q_cmn.SQL.Clear;
            ...
            FieldByName('kod_pgr').AsString+','+
            ''''+gr_name+''','+
            ''''+FieldByName('luk_dog_id').AsString+''','+
            FieldByName('luk_plt_id').AsString+','+
            ''''+FieldByName('snp_dog_id').AsString+''','+
            FieldByName('snp_plt_id').AsString+','+
            ''''+FieldByName('kod_npr').AsString+''','+
            FloatToStr(FieldByName('ves').AsFloat/1000)+','+
            FloatToStr(FieldByName('sum').AsFloat/1000)+
            ')');
            q_cmn.ExecSQL;
            next;
          end;
...
procedure Tf_f42.q_fact_plat_oraBeforeOpen(DataSet: TDataSet);
begin
  q_fact_plat_ora.SetVariable(':BEG_DATE',date1.date);
  q_fact_plat_ora.SetVariable(':END_DATE',date2.date);
  q_fact_plat_ora.SQL.SaveToFile('c:\1.sql');
end;

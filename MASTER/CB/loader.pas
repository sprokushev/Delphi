...
Begin
    ...
    While( StrUtils.LeftStr( re.lines[ i ], 4 ) <> 'Дата' ) And ( i <= re.Lines.Count ) Do
        Begin
            s2 := trim( re.lines[ i ] );

            If s2 <> '' Then
                Begin //не пустая строка
                    summ := GetVal( s2, 2, 3 );
                    ...
                    If empty Then
                        Begin //нет такой записи
                            ...
                            q1.Execute;
                            WriteEvent(
                                DateTimeToStr( Now ) + ': Добавлена запись в Выписку ' + inttostr(
                                    i + 1 ) + ' строка ' + inttostr( i ) + '.',
                                3 );
                            q1.SQL.Clear;
                            q1.Session.Commit;
                            ...
                            If empty Then
                                Begin //нет такой записи
                                    ...
                                    If length( trim( re.Lines[ ( i * 21 + 4 ) + (
                                                                   i * 3 ) + 14 ] ) ) > 12 Then          //ИНН больше 12 символов
                                    ...
                                    If length( trim( re.Lines[ ( i * 21 + 4 ) + ( i * 3 ) + 8 ] ) ) > 12 Then
                                        Begin                                                            //ИНН больше 12 символов
                                            WriteEvent(
                                                'Строка ' + trim(
                                                    inttostr( ( i * 21 + 4 ) + (
                                                                  i * 3 ) + 8 ) ) + ': Неверный ИНН ' + trim(
                                    ...
                                    q1.Session.Commit;
                                    WriteEvent(
                                        DateTimeToStr( Now ) + ': Добавлена запись в Выписку ' + inttostr( i + 1 )
                                            + ' строки c '
                                            + inttostr( ( i * 21 + 4 ) + ( i * 3 ) ) + ' по ' + inttostr( ( i * 21 + 4 )
                                                                                                              + ( i
                                                                                                                      * 3 )
                                                                                                              + 21 )
                                            + '.',
                        ...
                        Begin
                            ...
                            If empty Then
                                Begin //нет такой записи
                                    ...
                                    q1.SQL.add( 'BO_BANK_S, BO_BANK_R, BO_NUM, BO_DT1, BO_DT2,' );
                                    q1.SQL.add(
                                        'BO_OCHER, BO_RESERV, BO_NAZN, BO_VID, BO_RN_PARUS,' ); //bo_reserv будет ЛУКОЙЛовский код
                                    q1.SQL.add( 'ID, ID_DGV, BO_NUMSTR, ACCKEY, ID_CODE)' );
                                    q1.SQL.add( 'values (' );
                                    q1.SQL.add(
                                        'TO_DATE(''' + FormatDateTime( 'dd.mm.yyyy',
                                    ...
                                    WriteEvent(
                                        DateTimeToStr( Now ) + ': Добавлена запись в Выписку ' + inttostr(
                                            i + 1 ) + ' строки c ' + inttostr( ( i * 37 + 15 ) ) + ' по ' + inttostr( (
                                                                                                                          i
                                                                                                                              * 37
                                                                                                                              + 15 )
                                                                                                                          + 18 )
...
Begin
    ...
    If oraDScb.Connected Then
        Begin
            ...
            WriteEvent(
                DateTimeToStr(
                    Now ) + ': Вход в систему (' + oraDScb.LogonUsername + '@' + oraDScb.LogonDatabase + ').',
                44 );
...
Begin
    WriteEvent(
        DateTimeToStr( Now ) + ': Выход из системы (' + oraDScb.LogonUsername + '@' + oraDScb.LogonDatabase + ').', 7 );
    pbStatus.Visible := True;
    pbStatus.Min := 0;
    pbStatus.Max := 5;
...
Begin
    ...
    WriteEvent(
        DateTimeToStr( Now ) + ': Выбран счет ' + oraACC.Fieldbyname(
            'bv_acc' ).asString + ' за ' + FormatDateTime( 'dd.mm.yyyy', edDate.Date ) + '.',
        11 );
    pbStatus.Visible := True;
    pbStatus.Min := 0;
    pbStatus.Max := 3;
    ...
    For i := 1 To DBGridEh2.Columns.Count - 1 Do DBGridEh2.Columns[ i ].Title.Caption
        := 'Выписка по счету ' + lcACC.DisplayValue + ' за ' + edDate.text + DBGridEh2.Columns[ i ].Field.DisplayLabel;
...
Begin
    If MessageDlg( 'По данному счету, за эту дату данные уже присутствуют. Удаляем их?', mtConfirmation, [ mbYes,
                                                                                                           mbNo ], 0 ) =
        mrYes Then
        Begin // Удаляем данные
            //Удаляем из BANKDOCS
            ...
            q1.Session.Commit;
            WriteEvent(
                DateTimeToStr( Now ) + ': Удалены записи для счета ' + oraACC.Fieldbyname(
                    'bv_acc' ).asString + ' за ' + FormatDateTime( 'dd.mm.yyyy', edDate.Date ) + '.',
                16 );
        End;
...
oraPayNN.SetVariable( ':doctype', 'П/П' );
oraPayNN.Execute;
...
orapayFU.Open;
//Меняем номер за выбранный день
...
oraPayUPD.SetVariable(':sDOCTYPE','П/П');
        ...
        Begin
          ...
          if (trim(adoCBDB.FieldByName('#A_ACC').AsString) = trim(sACC)) or (trim(adoCBDB.FieldByName('#C_ACC').AsString)=trim(sACC)) then
            Begin //нет такой записи
             ...
             q1.Execute;
             WriteEvent( DateTimeToStr( Now ) + ': Добавлена запись в Выписку ' + inttostr(
                         i + 1 ) + ' строка ' + inttostr( i ) + '.', 3 );
             q1.SQL.Clear;
           End; //нет такой записи

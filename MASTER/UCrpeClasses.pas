...
Supports : Delphi 2, 3, 4 and 5
History  : Initial Version November, 1995 Created by John B. Murphy
Copyright (c) 2001 Crystal Decisions Inc.
*)
...
{------------------------------------------------------------------------------}
  ...
  protected
    ...
    function  GetMaxNLines : TStrings;
    procedure SetMaxNLines (const Value: TStrings);
    ...
    procedure OnChangeMaxNLines (Sender: TObject);
...
begin
  ...
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectInfo(TCrpe(Cx).JobNumber, Handle, objectInfo) then
  begin
...
begin
  ...
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectInfo(JobNum, Handle, objectInfo) then
  begin
  ...
  begin
    ...
    TCrpe(Cx).SetDllHandle;
    if not PESetObjectInfo(JobNum, Handle, objectInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(JobNum,errNoOption,errEngine,'',
...
begin
  ...
  if TCrpe(Cx).Version.Crpe.Major < 7 then Exit;

  if Parent.Parent.Parent is TCrpeEmbeddedFieldsItem then
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetNthEmbeddedFieldFormat (TCrpe(Cx).JobNumber, Handle,
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SUPPRESS_IF_ZERO,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetNthEmbeddedFieldFormat (TCrpe(Cx).JobNumber, Handle,
        FFieldN, PE_OFN_SUPPRESS_IF_ZERO, valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SUPPRESS_IF_ZERO,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_NEGATIVE_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_NEGATIVE_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_THOUSANDSSEPARATORS,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_THOUSANDSSEPARATORS,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_LEAD_ZERO,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_LEAD_ZERO,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_N_DECIMAL_PLACES,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_N_DECIMAL_PLACES,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_ROUNDING_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_ROUNDING_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  if not (FFieldType in [fvInt8s..fvCurrency]) then Exit;
  if not StatusIsGo then Exit;
  {Only applies to SCR8+}
  if TCrpe(Cx).Version.Crpe.Major < 8 then Exit;
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_REVERSE_SIGN_FOR_DISPLAY,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_REVERSE_SIGN_FOR_DISPLAY,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_ACCOUNTING_FORMAT,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_ACCOUNTING_FORMAT,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CURRENCY_SYMBOL,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CURRENCY_SYMBOL,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CURRENCY_SYMBOL_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CURRENCY_SYMBOL_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_ONE_CURRENCY_SYMBOL_PER_PAGE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_ONE_CURRENCY_SYMBOL_PER_PAGE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CURRENCY_POSITION_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CURRENCY_POSITION_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_THOUSAND_SYMBOL,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_THOUSAND_SYMBOL,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DECIMAL_SYMBOL,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DECIMAL_SYMBOL,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SHOW_ZERO_VALUE_AS,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SHOW_ZERO_VALUE_AS,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_ALLOW_FIELD_CLIPPING,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_ALLOW_FIELD_CLIPPING,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_WINDOWS_DEFAULT_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_ORDER, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_YEAR_TYPE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_MONTH_TYPE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_TYPE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_TYPE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_FIRST_SEPARATOR,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_SECOND_SEPARATOR,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_SEPARATOR,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_POSITION,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_ENCLOSURE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_ERA_TYPE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_CALENDAR_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_PREFIX_SEPARATOR,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_SUFFIX_SEPARATOR,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_WINDOWS_DEFAULT_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_ORDER, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_YEAR_TYPE, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_MONTH_TYPE, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_TYPE, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_TYPE, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_FIRST_SEPARATOR,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_SECOND_SEPARATOR,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_SEPARATOR,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_POSITION,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DAY_OF_WEEK_ENCLOSURE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_ERA_TYPE, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_CALENDAR_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_PREFIX_SEPARATOR,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_SUFFIX_SEPARATOR,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_TIME_BASE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_AM_PM_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_HOUR_TYPE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_MINUTE_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SECOND_TYPE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_AM_STRING,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_PM_STRING,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_HOUR_MINUTE_SEPARATOR,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_MINUTE_SECOND_SEPARATOR,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_TIME_BASE, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_TIME_BASE, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_AM_PM_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_HOUR_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_HOUR_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_MINUTE_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SECOND_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_AM_STRING, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_AM_STRING, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_PM_STRING, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_PM_STRING, ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_HOUR_MINUTE_SEPARATOR,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_MINUTE_SECOND_SEPARATOR,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_TIME_ORDER, ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_TIME_SEPARATOR_STRING,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_TIME_ORDER,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_TIME_ORDER,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_DATE_TIME_SEPARATOR_STRING,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
{ShowMessage: Cannot be retrieved..Print Engine error/bug}
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_FIRST_LINE_INDENT,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
...
begin
  ...
  if not (FFieldType in [fvString..fvPersistentMemo]) then Exit;
  if not StatusIsGo then Exit;
  if TCrpe(Cx).Version.Crpe.Major < 6 then Exit;
  TObjItem := nil;
  Changed := False;
...
{ Cannot be retrieved - Print Engine bug }
  ...
  begin
    ...
    begin
      ...
      begin
        TCrpe(Cx).SetDllHandle;
        if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_FIRST_LINE_INDENT,
          ValueInfo) then
        begin
          case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
...
{ShowMessage: Cannot be retrieved..Print Engine error/bug}
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_LEFT_INDENT,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
...
{ Cannot be retrieved - Print Engine bug }
  ...
  begin
    ...
    begin
      ...
      begin
        TCrpe(Cx).SetDllHandle;
        if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_LEFT_INDENT,
          ValueInfo) then
        begin
          case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
...
{ShowMessage: Cannot be retrieved..Print Engine error/bug}
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_RIGHT_INDENT,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
...
{ Cannot be retrieved - Print Engine bug }
  ...
  begin
    ...
    begin
      ...
      begin
        TCrpe(Cx).SetDllHandle;
        if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_RIGHT_INDENT,
          ValueInfo) then
        begin
          case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_TEXT_INTERPRETATION,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_TEXT_INTERPRETATION,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_LINE_SPACING_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_LINE_SPACING_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_LINE_SPACING,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_LINE_SPACING,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CHARACTER_SPACING,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_CHARACTER_SPACING,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
    ...
    case FFieldType of
      ...
      fvString :
        begin
          ...
          {MaxNLines}
          Name := ffnMaxNLines;
          TCrpeFieldFormatFormulas(Source).Name := ffnMaxNLines;
          Formula.Assign(TCrpeFieldFormatFormulas(Source).Formula);
          {TextInterpretation}
...
begin
  ...
  {Some formulas only apply to SCR8+}
  case FName of
    ffnTextInterpretation,
    ffnReverseSignForDisplay,
    ffnDayOfWeekEnclosure : if (TCrpe(Cx).Version.Crpe.Major < 8) then Exit;
  end;
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetNthEmbeddedFieldFormatFormula (TCrpe(Cx).JobNumber, Handle,
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormatFormula(TCrpe(Cx).JobNumber, Handle, iName, hText, iText) then
    begin
      TStringList(FFormula).OnChange := OnChangeStrings;
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
        TCrpe(Cx).BuildErrorString(Self) + 'GetFormula - ' +
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetNthEmbeddedFieldFormatFormula(TCrpe(Cx).JobNumber, Handle,
    ...
    begin
      TCrpe(Cx).SetDLLHandle;
      if not PESetObjectFormatFormula(TCrpe(Cx).JobNumber, Handle, iName, PChar(sVCL)) then
      begin
        TStringList(FFormula).OnChange := OnChangeStrings;
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'SetFormula - ' +
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_SYSTEM_DEFAULT_FORMATTING,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_USE_SYSTEM_DEFAULT_FORMATTING,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_BOOLEAN_OUPUT_TYPE,
      ValueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_BOOLEAN_OUPUT_TYPE,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  {Field Object Info}
  TCrpe(Cx).SetDllHandle;
  if not PEGetFieldObjectInfo(TCrpe(Cx).JobNumber, Handle, fieldObjectInfo) then
  begin
...
begin
  if not StatusIsGo then Exit;
  if TCrpe(Cx).Version.Crpe.Major < 7 then Exit;
  {Requires 7.0 maintenance release or higher (7.x.100.15)}
  if (TCrpe(Cx).Version.Crpe.Major = 7) and (TCrpe(Cx).Version.Crpe.Release = 0) then
    Exit;

  TCrpe(Cx).SetDllHandle;
  if not PESetObjectNthHiliteConditionPriority(TCrpe(Cx).JobNumber, Handle,
    FIndex, nIndex) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  TCrpe(Cx).SetDLLHandle;
  if not PEGetObjectNthHiliteCondition(TCrpe(Cx).JobNumber, Handle, FIndex, HiliteInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  begin
    ...
    {Send to Print Engine}
    TCrpe(Cx).SetDLLHandle;
    if not PESetObjectNthHiliteCondition (TCrpe(Cx).JobNumber, Handle, FIndex, HiliteInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    TCrpe(Cx).SetDllHandle;
    if not PEClearObjectHiliteConditions(TCrpe(Cx).JobNumber, Handle) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  TCrpe(Cx).SetDLLHandle;
  nConditions := PEGetObjectNHiliteConditions(TCrpe(Cx).JobNumber, Handle);
...
begin
  ...
  {Add the Condition}
  TCrpe(Cx).SetDLLHandle;
  if not PEAddObjectHiliteCondition(TCrpe(Cx).JobNumber, Handle, HiliteInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  TCrpe(Cx).SetDLLHandle;
  if not PERemoveObjectNthHiliteCondition(TCrpe(Cx).JobNumber, Handle, nIndex) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  TCrpe(Cx).SetDLLHandle;
  if not PEGetObjectNthHiliteCondition(TCrpe(Cx).JobNumber, Handle, nIndex, HiliteInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  if TCrpe(Cx).Version.Crpe.Major > 7 then
...
begin
  ...
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectFontColor(TCrpe(Cx).JobNumber, Handle, FontColorInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    TCrpe(Cx).SetDllHandle;
    if not PESetObjectFontColor(TCrpe(Cx).JobNumber, Handle, FontColorInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
          TCrpe(Cx).BuildErrorString(Parent) + '[' + IntToStr(FIndex) +
...
begin
  ...
  if Parent.Parent is TCrpeSubreports then
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormatFormula(xJob, Handle,
...
begin
  ...
  {Get FormatFormula Text}
  pText := GlobalLock(hText);
  FTightHorizontal.SetText(pText);
  GlobalUnlock(hText);
  GlobalFree(hText);
  Result := FTightHorizontal;
end;
...
begin
  ...
  {Get FormatFormula Text}
  pText := GlobalLock(hText);
  FBackgroundColor.SetText(pText);
  GlobalUnlock(hText);
  GlobalFree(hText);
  Result := FBackgroundColor;
end;
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDLLHandle;
      if not PESetObjectFormatFormula(xJob, Handle,
...
begin
  ...
  if Self.Parent.Parent is TCrpeSubreports then
    xJob := TCrpe(Cx).PrintJobs(0)
  else
    xJob := TCrpe(Cx).JobNumber;
...
begin
  ...
  begin
    TCrpe(Cx).SetDLLHandle;
    if not PEGetObjectFormatFormula(xJob, Handle, PE_FFN_DROP_SHADOW, hText, iText) then
    begin
      TStringList(FDropShadow).OnChange := OnChangeDropShadow;
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDLLHandle;
    if not PEGetObjectFormatFormula(xJob, Handle, PE_FFN_FORE_COLOR, hText, iText) then
    begin
      TStringList(FForeColor).OnChange := OnChangeForeColor;
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDLLHandle;
      if not PESetObjectFormatFormula(xJob, Handle, PE_FFN_FORE_COLOR, PChar(sVCL)) then
      begin
        TStringList(FForeColor).OnChange := OnChangeForeColor;
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDLLHandle;
      if not PESetObjectFormatFormula(xJob, Handle, PE_FFN_BACK_COLOR, PChar(sVCL)) then
      begin
        TStringList(FBackgroundColor).OnChange := OnChangeBackgroundColor;
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_LEFT_LINE_STYLE, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_LEFT_LINE_STYLE,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_RIGHT_LINE_STYLE, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_RIGHT_LINE_STYLE,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_TOP_LINE_STYLE, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_TOP_LINE_STYLE,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_BOTTOM_LINE_STYLE, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_BOTTOM_LINE_STYLE,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_TIGHT_HORIZONTAL, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_TIGHT_HORIZONTAL,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_DROP_SHADOW, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_DROP_SHADOW, valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_FORE_COLOR, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_FORE_COLOR,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_BACK_COLOR, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_BACK_COLOR,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_BACK_COLOR,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  {Suppress}
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_VISIBLE, valueInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  {KeepTogether}
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_KEEP_OBJECT_TOGETHER, valueInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
        TCrpe(Cx).BuildErrorString(Self) + 'GetSuppress <PEGetObjectFormat>') of
      errIgnore : Exit;
...
begin
  ...
  {CloseBorder}
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SPLIT_ADORNMENT, valueInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectFormatFormula(TCrpe(Cx).JobNumber, Handle,
...
begin
  ...
  if valueInfo.viBoolean = Bool(FSuppress) then
  begin
    ...
    TCrpe(Cx).SetDllHandle;
    if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_VISIBLE, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  {KeepTogether}
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_KEEP_OBJECT_TOGETHER,
    valueInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  if valueInfo.viBoolean <> Bool(FKeepTogether) then
  begin
    valueInfo.viBoolean := Bool(FKeepTogether);
    TCrpe(Cx).SetDllHandle;
    if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_KEEP_OBJECT_TOGETHER,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  {CloseBorder}
  TCrpe(Cx).SetDllHandle;
  if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SPLIT_ADORNMENT,
    valueInfo) then
  begin
    case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
  ...
  if valueInfo.viBoolean <> Bool(FCloseBorder) then
  begin
    valueInfo.viBoolean := Bool(FCloseBorder);
    TCrpe(Cx).SetDllHandle;
    if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SPLIT_ADORNMENT,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  if CompareStr(sText, FToolTip) <> 0 then
  begin
    TCrpe(Cx).SetDllHandle;
    if not PESetObjectFormatFormula(TCrpe(Cx).JobNumber, Handle, PE_FFN_TOOL_TIP_TEXT,
      PChar(FToolTip)) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_VISIBLE, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'GetSuppress <PEGetObjectFormat>') of
        errIgnore : Exit;
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_VISIBLE, valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
            TCrpe(Cx).BuildErrorString(Self) + 'SetSuppress <PESetObjectFormat>') of
          errIgnore : Exit;
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_KEEP_OBJECT_TOGETHER, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_KEEP_OBJECT_TOGETHER,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'SetKeepTogether <PEGetObjectFormat>') of
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_KEEP_OBJECT_TOGETHER,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
            TCrpe(Cx).BuildErrorString(Self) + 'SetKeepTogether <PESetObjectFormat>') of
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_SPLIT_ADORNMENT, valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'GetCloseBorder <PEGetObjectFormat>') of
        errIgnore : Exit;
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_SPLIT_ADORNMENT,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'SetCloseBorder <PEGetObjectFormat>') of
  ...
  {They are opposite so we must check if they ARE equal to see a difference}
  if valueInfo.viBoolean = Bool(FCloseBorder) then
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_SPLIT_ADORNMENT,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
            TCrpe(Cx).BuildErrorString(Self) + 'SetCloseBorder <PESetObjectFormat>') of
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormatFormula(xJob, Handle, PE_FFN_TOOL_TIP_TEXT,
        PChar(FToolTip)) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
            TCrpe(Cx).BuildErrorString(Self) + 'SetToolTip <PESetObjectFormatFormula>') of
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_HORALIGNMENT,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  if valueInfo.viInteger <> Ord(FAlignment) then
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_HORALIGNMENT,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SUPPRESS_IF_DUPLICATED,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  if valueInfo.viBoolean <> Bool(FSuppressIfDuplicated) then
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(TCrpe(Cx).JobNumber, Handle, PE_OFN_SUPPRESS_IF_DUPLICATED,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PEGetObjectFormat(xJob, Handle, PE_OFN_EXPAND, valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PEGetObjectFormat(xJob, Handle, PE_OFN_EXPAND,
        valueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
    ...
    begin
      ...
      begin
        TCrpe(Cx).SetDllHandle;
        if not PESetObjectFormat(xJob, Handle, PE_OFN_EXPAND,
          valueInfo) then
        begin
          case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PEGetObjectFormat(xJob, Handle, PE_OFN_MAX_N_LINES,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errCRPEBugs,errEngine,'',
...
begin
  ...
  {Compare}
  if ValueInfo.viInteger <> FMaxNLines then
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_MAX_N_LINES,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errCRPEBugs,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormat(xJob, Handle, PE_OFN_TEXT_ROTATION,
      valueInfo) then
    begin
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormat(xJob, Handle, PE_OFN_TEXT_ROTATION,
        ValueInfo) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  begin
    TCrpe(Cx).SetDLLHandle;
    if not PESetObjectFormatFormula(TCrpe(Cx).JobNumber, Handle,
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PESetObjectFormatFormula(TCrpe(Cx).JobNumber, Handle, PE_FFN_HYPERLINK, PChar(sVCL)) then
    begin
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errNoOption,errEngine,'',
...
begin
  ...
  TStringList(FMaxNLines).OnChange := OnChangeMaxNLines;
...
begin
  ...
  TStringList(FMaxNLines).OnChange := nil;
...
begin
  ...
  begin
    ...
    MaxNLines.Assign(TCrpeFormatFormulasB(Source).MaxNLines);
...
{ OnChangeMaxNLines                                                            }
{------------------------------------------------------------------------------}
procedure TCrpeFormatFormulasB.OnChangeMaxNLines (Sender: TObject);
begin
  if Sender is TStringList then
  begin
    TStringList(Sender).OnChange := nil;
    xFormula.Assign(TStringList(Sender));
    SetMaxNLines(xFormula);
    TStringList(Sender).OnChange := OnChangeMaxNLines;
  end;
end;
{------------------------------------------------------------------------------}
...
begin
  ...
  begin
    TCrpe(Cx).SetDLLHandle;
    if not PEGetObjectFormatFormula(xJob, Handle, PE_FFN_OBJECT_VISIBILITY,
      hText, iText) then
    begin
      TStringList(FSuppress).OnChange := OnChangeSuppress;
      case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
...
begin
  ...
  {If we are getting the Format for a Subreport, we need to use
   the main Report job number}
  if Parent.Parent is TCrpeSubreports then
    xJob := TCrpe(Cx).PrintJobs(0)
  else
    xJob := TCrpe(Cx).JobNumber;
...
begin
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetObjectFormatFormula(xJob, Handle, PE_FFN_HYPERLINK, PChar(sVCL)) then
      begin
        case TCrpe(Cx).GetErrorMsg(xJob,errNoOption,errEngine,'',
            TCrpe(Cx).BuildErrorString(Self) + 'SetHyperLink <PESetObjectFormatFormula>') of
          errIgnore : Exit;
...
{ GetMaxNLines                                                                 }
{------------------------------------------------------------------------------}
function TCrpeFormatFormulasB.GetMaxNLines : TStrings;
var
  hText : HWnd;
  iText : Smallint;
  pText : PChar;
...
begin
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetNthEmbeddedFieldFormatFormula (TCrpe(Cx).JobNumber, Handle,
      FFieldN, PE_FFN_MAX_N_LINES, hText, iText) then
    begin
      TStringList(FMaxNLines).OnChange := OnChangeMaxNLines;
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'GetMaxNLines <PEGetNthEmbeddedFieldFormatFormula>') of
        errIgnore : Exit;
        errAbort  : Abort;
        errRaise  : raise ECrpeError.Create(TCrpe(Cx).LastErrorNumber, TCrpe(Cx).LastErrorString);
      end;
  ...
  begin
    TCrpe(Cx).SetDllHandle;
    if not PEGetObjectFormatFormula(xJob, Handle,
      PE_FFN_MAX_N_LINES, hText, iText) then
    begin
      TStringList(FMaxNLines).OnChange := OnChangeMaxNLines;
      case TCrpe(Cx).GetErrorMsg(xJob,errCRPEBugs,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'GetMaxNLines <PEGetObjectFormatFormula>') of
        errIgnore : Exit;
        errAbort  : Abort;
        errRaise  : raise ECrpeError.Create(TCrpe(Cx).LastErrorNumber, TCrpe(Cx).LastErrorString);
      end;
...
{ SetMaxNLines                                                                 }
{------------------------------------------------------------------------------}
procedure TCrpeFormatFormulasB.SetMaxNLines (const Value: TStrings);
var
  hText      : HWnd;
  iText      : Smallint;
  pText      : PChar;
...
begin
  ...
  begin
    ...
    begin
      TStringList(FMaxNLines).OnChange := OnChangeMaxNLines;
      case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'SetMaxNLines <PEGetNthEmbeddedFieldFormatFormula>') of
        errIgnore : Exit;
        errAbort  : Abort;
        errRaise  : raise ECrpeError.Create(TCrpe(Cx).LastErrorNumber, TCrpe(Cx).LastErrorString);
      end;
  ...
  begin
    ...
    begin
      TStringList(FMaxNLines).OnChange := OnChangeMaxNLines;
      case TCrpe(Cx).GetErrorMsg(xJob,errCRPEBugs,errEngine,'',
          TCrpe(Cx).BuildErrorString(Self) + 'SetMaxNLines <PEGetObjectFormatFormula>') of
        errIgnore : Exit;
        errAbort  : Abort;
        errRaise  : raise ECrpeError.Create(TCrpe(Cx).LastErrorNumber, TCrpe(Cx).LastErrorString);
      end;
  ...
  begin
    ...
    begin
      TCrpe(Cx).SetDllHandle;
      if not PESetNthEmbeddedFieldFormatFormula(TCrpe(Cx).JobNumber, Handle,
        FFieldN, PE_FFN_MAX_N_LINES, PChar(sVCL)) then
      begin
        TStringList(FMaxNLines).OnChange := OnChangeMaxNLines;
        case TCrpe(Cx).GetErrorMsg(TCrpe(Cx).JobNumber,errCRPEBugs,errEngine,'',
            TCrpe(Cx).BuildErrorString(Self) + 'SetMaxNLines <PESetNthEmbeddedFieldFormatFormula>') of
          errIgnore : Exit;
          errAbort  : Abort;
          errRaise  : raise ECrpeError.Create(TCrpe(Cx).LastErrorNumber, TCrpe(Cx).LastErrorString);
        end;
    ...
    begin
      TCrpe(Cx).SetDLLHandle;
      if not PESetObjectFormatFormula(xJob, Handle,
        PE_FFN_MAX_N_LINES, PChar(sVCL)) then
      begin
        TStringList(FMaxNLines).OnChange := OnChangeMaxNLines;
        case TCrpe(Cx).GetErrorMsg(xJob,errCRPEBugs,errEngine,'',
            TCrpe(Cx).BuildErrorString(Self) + 'SetMaxNLines <PESetObjectFormatFormula>') of
          errIgnore : Exit;
          errAbort  : Abort;
          errRaise  : raise ECrpeError.Create(TCrpe(Cx).LastErrorNumber, TCrpe(Cx).LastErrorString);
        end;
...
begin
  Result := FSuppressIfDuplicated;
  {SuppressIfDuplicated formula not applicable to some classes}
  if (Parent.Parent is TCrpeTextObjects) or
     (Parent.Parent is TCrpeSubreports) then Exit;
  if not StatusIsGo then Exit;
  ...
  {Get FormatFormula Text}
  pText := GlobalLock(hText);
  FSuppressIfDuplicated.SetText(pText);
  GlobalUnlock(hText);
  GlobalFree(hText);
  Result := FSuppressIfDuplicated;
end;
...
begin
  ...
  if (Parent.Parent is TCrpeTextObjects) or
     (Parent.Parent is TCrpeSubreports) then
     Exit;

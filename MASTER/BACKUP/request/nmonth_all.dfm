object frm_MonthNew: Tfrm_MonthNew
  Left = 322
  Top = 250
  Width = 635
  Height = 523
  Caption = #1053#1086#1074#1072#1103' '#1047#1072#1087#1080#1089#1100
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 5
    Top = 77
    Width = 69
    Height = 13
    Caption = #1050#1086#1085#1090#1088#1072#1075#1077#1085#1090
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 5
    Top = 103
    Width = 52
    Height = 13
    Caption = #1044#1086#1075#1086#1074#1086#1088
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 5
    Top = 126
    Width = 50
    Height = 13
    Caption = #1055#1088#1086#1076#1091#1082#1090
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label4: TLabel
    Left = 5
    Top = 199
    Width = 50
    Height = 13
    Caption = #1057#1090#1072#1085#1094#1080#1103
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label5: TLabel
    Left = 5
    Top = 153
    Width = 39
    Height = 13
    Caption = #1052#1077#1089#1103#1094
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label6: TLabel
    Left = 5
    Top = 174
    Width = 58
    Height = 13
    Caption = #1047#1072#1103#1074#1083#1077#1085#1086
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label7: TLabel
    Left = 308
    Top = 29
    Width = 76
    Height = 13
    Caption = #1042#1093#1086#1076#1103#1097#1080#1081' '#8470
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label8: TLabel
    Left = 5
    Top = 223
    Width = 70
    Height = 13
    Caption = #1042#1080#1076' '#1086#1087#1083#1072#1090#1099
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label9: TLabel
    Left = 5
    Top = 265
    Width = 36
    Height = 13
    Caption = #1042#1077#1090#1082#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label10: TLabel
    Left = 5
    Top = 289
    Width = 70
    Height = 13
    Caption = #1055#1086#1083#1091#1095#1072#1090#1077#1083#1100
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label11: TLabel
    Left = 5
    Top = 333
    Width = 78
    Height = 13
    Caption = #1055#1086#1090#1088#1077#1073#1080#1090#1077#1083#1100
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label12: TLabel
    Left = 5
    Top = 243
    Width = 80
    Height = 13
    Caption = #1042#1080#1076' '#1086#1090#1075#1088#1091#1079#1082#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label13: TLabel
    Left = 324
    Top = 221
    Width = 71
    Height = 13
    Caption = #1059#1089#1083' '#1086#1087#1083#1072#1090#1099
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label14: TLabel
    Left = 5
    Top = 56
    Width = 48
    Height = 13
    Caption = #1060#1080#1083#1080#1072#1083
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label15: TLabel
    Left = 398
    Top = 146
    Width = 31
    Height = 13
    Caption = #1062#1077#1085#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label16: TLabel
    Left = 5
    Top = 310
    Width = 70
    Height = 13
    Caption = #1055#1086#1083#1091#1095#1072#1090#1077#1083#1100
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label17: TLabel
    Left = 5
    Top = 356
    Width = 78
    Height = 13
    Caption = #1055#1086#1090#1088#1077#1073#1080#1090#1077#1083#1100
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object sbPoluch: TRxSpeedButton
    Left = 594
    Top = 282
    Width = 22
    Height = 22
    Action = acSeekPoluch
    Glyph.Data = {
      
      1F7C1F7C1F7C}
  end
  object sbPotreb: TRxSpeedButton
    Left = 594
    Top = 328
    Width = 22
    Height = 22
    Action = acSeekPotreb
    Glyph.Data = {
      
      1F7C1F7C1F7C}
  end
  object Label18: TLabel
    Left = 5
    Top = 381
    Width = 81
    Height = 13
    Caption = #1055#1086#1079'.'#1055#1083'.'#1055#1086#1089#1090'.'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label19: TLabel
    Left = 5
    Top = 29
    Width = 91
    Height = 13
    Caption = #1048#1089#1093'.'#8470' '#1082#1083#1080#1077#1085#1090#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label20: TLabel
    Left = 5
    Top = 6
    Width = 87
    Height = 13
    Caption = #1057#1090#1072#1090#1091#1089' '#1079#1072#1082#1072#1079#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label21: TLabel
    Left = 230
    Top = 172
    Width = 11
    Height = 13
    Caption = #1090#1085
  end
  object Label22: TLabel
    Left = 607
    Top = 172
    Width = 12
    Height = 13
    Caption = #1074#1094
  end
  object Label23: TLabel
    Left = 556
    Top = 148
    Width = 33
    Height = 13
    Caption = #1088#1091#1073'/'#1090#1085
  end
  object Label24: TLabel
    Left = 345
    Top = 166
    Width = 90
    Height = 26
    Caption = #1074' '#1090'.'#1095'. '#1089#1088#1086#1095#1085#1099#1077' '#1087#1077#1088#1077#1074#1086#1079#1082#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    WordWrap = True
  end
  object Label25: TLabel
    Left = 526
    Top = 171
    Width = 11
    Height = 13
    Caption = #1090#1085
  end
  object Label26: TLabel
    Left = 327
    Top = 171
    Width = 12
    Height = 13
    Caption = #1074#1094
  end
  object Label27: TLabel
    Left = 5
    Top = 404
    Width = 74
    Height = 13
    Caption = #1055#1088#1080#1084#1077#1095#1072#1085#1080#1077
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label28: TLabel
    Left = 5
    Top = 428
    Width = 49
    Height = 13
    Caption = #1043#1088#1072#1092#1072' 4'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label29: TLabel
    Left = 209
    Top = 147
    Width = 79
    Height = 13
    Caption = #1044#1077#1081#1089#1090#1074#1091#1077#1090' '#1089':'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object cbPredpr: TRxDBLookupCombo
    Left = 105
    Top = 75
    Width = 512
    Height = 21
    DropDownCount = 20
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1082#1086#1085#1090#1088#1072#1075#1077#1085#1090#1072'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'SHORT_NAME'
    LookupSource = dsPredpr
    TabOrder = 6
  end
  object cbDog: TRxDBLookupCombo
    Left = 105
    Top = 98
    Width = 512
    Height = 21
    DropDownCount = 8
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1076#1086#1075#1086#1074#1086#1088'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'DOG_NUMBER'
    LookupSource = dsDog
    TabOrder = 7
    OnChange = cbDogChange
  end
  object cbProd: TRxDBLookupCombo
    Left = 105
    Top = 121
    Width = 512
    Height = 21
    DropDownCount = 12
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1087#1088#1086#1076#1091#1082#1090'>'
    EmptyValue = '0'
    LookupField = 'ID_NPR'
    LookupDisplay = 'LONG_NAME_NPR'
    LookupSource = dsProd
    TabOrder = 8
  end
  object cbStan: TRxDBLookupCombo
    Left = 232
    Top = 193
    Width = 385
    Height = 21
    DropDownCount = 20
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1089#1090#1072#1085#1094#1080#1102'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'STAN_NAME'
    LookupSource = dsStan
    TabOrder = 16
    OnChange = cbStanChange
  end
  object edReq: TRxCalcEdit
    Left = 105
    Top = 169
    Width = 120
    Height = 21
    AutoSize = False
    DisplayFormat = ',0.000'
    FormatOnEditing = True
    NumGlyphs = 2
    TabOrder = 12
    OnChange = edReqChange
  end
  object BitBtn1: TBitBtn
    Left = 245
    Top = 456
    Width = 75
    Height = 25
    Action = acSave
    Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
    Default = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 28
  end
  object BitBtn2: TBitBtn
    Left = 341
    Top = 455
    Width = 75
    Height = 25
    Action = acExit
    Cancel = True
    Caption = #1042#1099#1093#1086#1076
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 29
  end
  object edNum: TEdit
    Left = 393
    Top = 26
    Width = 89
    Height = 21
    TabOrder = 3
  end
  object cbPayForm: TRxDBLookupCombo
    Left = 105
    Top = 217
    Width = 208
    Height = 21
    DropDownCount = 8
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1074#1080#1076' '#1086#1087#1083#1072#1090#1099'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'PAYFORM_NAME'
    LookupSource = dsPayform
    TabOrder = 17
  end
  object cbVetka: TRxDBLookupCombo
    Left = 105
    Top = 259
    Width = 512
    Height = 21
    DropDownCount = 20
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1074#1077#1090#1082#1072'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'VETKA_NAME'
    LookupSource = dsVetka
    TabOrder = 20
  end
  object cbPoluch: TRxDBLookupCombo
    Left = 105
    Top = 283
    Width = 488
    Height = 21
    DropDownCount = 20
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1087#1086#1083#1091#1095#1072#1090#1077#1083#1103'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'SHORT_NAME'
    LookupSource = dsPoluch
    TabOrder = 21
  end
  object cbPotreb: TRxDBLookupCombo
    Left = 105
    Top = 328
    Width = 480
    Height = 21
    DropDownCount = 20
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1087#1086#1090#1088#1077#1073#1080#1090#1077#1083#1103'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'SHORT_NAME'
    LookupSource = dsPotreb
    TabOrder = 23
  end
  object cbVidOtgr: TRxDBLookupCombo
    Left = 105
    Top = 237
    Width = 208
    Height = 21
    DropDownCount = 8
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1074#1080#1076' '#1086#1090#1075#1088#1091#1079#1082#1080'>'
    EmptyValue = '0'
    LookupField = 'LOAD_ABBR'
    LookupDisplay = 'LOAD_NAME'
    LookupSource = dsVidOtgr
    TabOrder = 19
  end
  object cbUslOpl: TRxDBLookupCombo
    Left = 408
    Top = 215
    Width = 209
    Height = 21
    DropDownCount = 8
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1091#1089#1083' '#1086#1087#1083#1072#1090#1099'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'NAME'
    LookupSource = dsUslOpl
    TabOrder = 18
  end
  object cbOrgstru: TRxDBLookupCombo
    Left = 105
    Top = 51
    Width = 512
    Height = 21
    DropDownCount = 12
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1055#1041#1045'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'NAME'
    LookupSource = dsOrg_Stru
    TabOrder = 5
  end
  object edPrice: TRxCalcEdit
    Left = 441
    Top = 144
    Width = 104
    Height = 21
    AutoSize = False
    DisplayFormat = ',0.00'
    FormatOnEditing = True
    NumGlyphs = 2
    TabOrder = 11
  end
  object edPoluch: TEdit
    Left = 105
    Top = 305
    Width = 512
    Height = 21
    MaxLength = 100
    TabOrder = 22
  end
  object edPotreb: TEdit
    Left = 104
    Top = 351
    Width = 513
    Height = 21
    MaxLength = 100
    TabOrder = 24
  end
  object edDatePlan: TDateTimePicker
    Left = 105
    Top = 144
    Width = 97
    Height = 21
    Date = 37839.812709131950000000
    Time = 37839.812709131950000000
    TabOrder = 9
    OnChange = edDatePlanChange
  end
  object edDat: TDateTimePicker
    Left = 489
    Top = 26
    Width = 97
    Height = 21
    Date = 37839.812709131950000000
    Time = 37839.812709131950000000
    TabOrder = 4
  end
  object cbPlanStru: TRxDBLookupCombo
    Left = 105
    Top = 376
    Width = 512
    Height = 21
    DropDownCount = 20
    EscapeClear = False
    DisplayEmpty = '<'#1042#1099#1073#1077#1088#1080#1090#1077' '#1087#1086#1079#1080#1094#1080#1102' '#1087#1083#1072#1085#1072' '#1087#1086#1089#1090#1072#1074#1086#1082'>'
    EmptyValue = '0'
    LookupField = 'ID'
    LookupDisplay = 'NAME'
    LookupSource = dsPlanstru
    TabOrder = 25
    OnChange = cbPlanStruChange
  end
  object edClientNum: TEdit
    Left = 105
    Top = 27
    Width = 89
    Height = 21
    TabOrder = 1
  end
  object cbStatusZakaz: TRxDBLookupCombo
    Left = 105
    Top = 2
    Width = 512
    Height = 21
    DropDownCount = 12
    EscapeClear = False
    LookupField = 'ID'
    LookupDisplay = 'NAME'
    LookupSource = ds_status_zakaz
    TabOrder = 0
  end
  object edClientDat: TDateTimePicker
    Left = 201
    Top = 26
    Width = 97
    Height = 21
    Date = 37839.812709131950000000
    Time = 37839.812709131950000000
    TabOrder = 2
  end
  object edKol: TRxCalcEdit
    Left = 249
    Top = 169
    Width = 72
    Height = 21
    AutoSize = False
    DisplayFormat = ',0'
    FormatOnEditing = True
    NumGlyphs = 2
    TabOrder = 13
  end
  object edSpeedVes: TRxCalcEdit
    Left = 433
    Top = 168
    Width = 88
    Height = 21
    AutoSize = False
    DisplayFormat = ',0.000'
    FormatOnEditing = True
    NumGlyphs = 2
    TabOrder = 14
    OnChange = edSpeedVesChange
  end
  object edSpeedKol: TRxCalcEdit
    Left = 545
    Top = 168
    Width = 56
    Height = 21
    AutoSize = False
    DisplayFormat = ',0'
    FormatOnEditing = True
    NumGlyphs = 2
    TabOrder = 15
  end
  object edPrich: TEdit
    Left = 104
    Top = 399
    Width = 513
    Height = 21
    MaxLength = 100
    TabOrder = 26
  end
  object edGR4: TEdit
    Left = 104
    Top = 423
    Width = 513
    Height = 21
    MaxLength = 100
    TabOrder = 27
  end
  object edBeginDate: TDateTimePicker
    Left = 297
    Top = 143
    Width = 97
    Height = 21
    Date = 37839.812709131950000000
    Time = 37839.812709131950000000
    TabOrder = 10
    OnChange = edDatePlanChange
  end
  object ed_StanKod: TEdit
    Left = 105
    Top = 194
    Width = 120
    Height = 21
    TabOrder = 30
    OnChange = ed_StanKodChange
  end
  object oraTSes: TOracleSession
    DesignConnection = True
    LogonUsername = 'master'
    LogonPassword = 'master'
    LogonDatabase = 'BUH'
    Left = 462
    Top = 381
  end
  object oraKls_predpr: TOracleDataSet
    SQL.Strings = (
      'SELECT  /*+ ALL_ROWS*/'
      '  kls.ID, kls.short_name'
      '  FROM kls_predpr kls WHERE is_block<>1'
      'order by kls.short_name')
    QBEDefinition.QBEFieldDefs = {
      030000000200000002000000494401000000000A00000053484F52545F4E414D
      450100000000}
    DetailFields = 'id'
    Session = oraTSes
    Left = 134
    Top = 239
  end
  object dsPredpr: TDataSource
    DataSet = oraKls_predpr
    Left = 174
    Top = 231
  end
  object oraKls_dog: TOracleDataSet
    SQL.Strings = (
      
        'SELECT kls_dog.ID, kls_dog.dog_number,kls_dog.dog_date,kls_dog.u' +
        'sl_opl_id'
      '  FROM kls_predpr kls, kls_dog kls_dog'
      ' WHERE ((kls.ID = kls_dog.predpr_id))'
      'and kls.id=:id')
    Variables.Data = {0300000001000000030000003A4944030000000000000000000000}
    QBEDefinition.QBEFieldDefs = {
      030000000300000002000000494401000000000A000000444F475F4E554D4245
      52010000000008000000444F475F444154450100000000}
    Master = oraKls_predpr
    MasterFields = 'id'
    Session = oraTSes
    Left = 133
    Top = 285
  end
  object dsDog: TDataSource
    DataSet = oraKls_dog
    Left = 189
    Top = 277
  end
  object oraKls_prod: TOracleDataSet
    SQL.Strings = (
      'SELECT * FROM KLS_PROD'
      'WHERE EXISTS '
      '(SELECT NULL '
      '  FROM KLS_PROD_GROUPS_DESC A, KLS_PROD_GROUPS B '
      ' WHERE A.PROD_GROUPS_ID=B.ID'
      '   AND B.PROD_TYPE_GRP_ID =4'
      '   AND A.PROD_ID_NPR=KLS_PROD.ID_NPR'
      '   )'
      'order by Name_npr'
      '')
    QBEDefinition.QBEFieldDefs = {
      
      0100000000}
    Session = oraTSes
    Left = 410
    Top = 201
  end
  object dsProd: TDataSource
    DataSet = oraKls_prod
    Left = 442
    Top = 201
  end
  object oraKls_stan: TOracleDataSet
    SQL.Strings = (
      'SELECT   kls_stan.ID, kls_stan.stan_name, kls_stan.stan_kod'
      '    FROM kls_stan '
      'ORDER BY kls_stan.stan_name ASC')
    QBEDefinition.QBEFieldDefs = {
      03000000020000000200000049440100000000090000005354414E5F4E414D45
      0100000000}
    Session = oraTSes
    Left = 420
    Top = 292
  end
  object dsStan: TDataSource
    DataSet = oraKls_stan
    Left = 460
    Top = 292
  end
  object oraQIns: TOracleQuery
    SQL.Strings = (
      'INSERT INTO month_all'
      
        '            (APPL_TAG,kls_predpr_id, kls_dog_id, kls_prod_id, da' +
        'te_dog, request, kol, speed_ves, speed_kol,'
      
        '             date_plan, stan_id, input_date, input_number, payfo' +
        'rm_id,'
      
        '             load_abbr, vetka_id, poluch_id, potreb_id,usl_opl_i' +
        'd,orgstru_id,price,'
      
        #9'poluch_name,potreb_name,kls_planstruid,is_agent,status_zakaz_id' +
        ',client_number,client_date,prich,gr4,begin_date,zakaz_old_id'
      '            )'
      
        '     VALUES ('#39'MASTER'#39',:kls_predpr_id, :kls_dog_id, :kls_prod_id,' +
        ' :date_dog, :request, :kol, :speed_ves, :speed_kol,'
      
        '             :date_plan, :stan_id, TRUNC(:input_date), :input_nu' +
        'mber, :payform_id,'
      
        '             :load_abbr, :vetka_id, :poluch_id, :potreb_id,:usl_' +
        'opl_id,:orgstru_id,:price,'
      
        #9':poluch_name,:potreb_name,:kls_planstruid,:is_agent,:status_zak' +
        'az_id,:client_number,:client_date,:prich,:gr4,:BEGIN_DATE,:ZAKAZ' +
        '_OLD_ID'
      '            )')
    Session = oraTSes
    Variables.Data = {
      
      003A5A414B415A5F4F4C445F4944030000000000000000000000}
    Left = 430
    Top = 381
  end
  object ActionManager1: TActionManager
    Images = f_main.img_Common
    Left = 365
    Top = 48
    StyleName = 'XP Style'
    object acSave: TAction
      Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
      OnExecute = acSaveExecute
    end
    object acExit: TAction
      Caption = #1042#1099#1093#1086#1076
      OnExecute = acExitExecute
    end
    object acSeekPoluch: TAction
      ImageIndex = 9
      OnExecute = acSeekPoluchExecute
    end
    object acSeekPotreb: TAction
      Caption = 'acSeekPotreb'
      ImageIndex = 9
      OnExecute = acSeekPotrebExecute
    end
  end
  object oraPayform: TOracleDataSet
    SQL.Strings = (
      'select id,'
      '       payform_name'
      '  from kls_payform'
      'order by payform_name')
    QBEDefinition.QBEFieldDefs = {
      030000000200000002000000494401000000000C000000504159464F524D5F4E
      414D450100000000}
    Session = oraTSes
    Left = 428
    Top = 330
  end
  object dsPayform: TDataSource
    DataSet = oraPayform
    Left = 468
    Top = 338
  end
  object oraVidOtgr: TOracleDataSet
    SQL.Strings = (
      'select load_abbr,'
      '       load_name'
      '  from kls_vid_otgr'
      'order by load_name')
    QBEDefinition.QBEFieldDefs = {
      0300000002000000090000004C4F41445F414242520100000000090000004C4F
      41445F4E414D450100000000}
    Session = oraTSes
    Left = 244
    Top = 263
  end
  object dsVidOtgr: TDataSource
    DataSet = oraVidOtgr
    Left = 276
    Top = 263
  end
  object oraVetka: TOracleDataSet
    SQL.Strings = (
      'select id,'
      '       vetka_name'
      '  from kls_vetka'
      'order by vetka_name')
    QBEDefinition.QBEFieldDefs = {
      030000000200000002000000494401000000000A0000005645544B415F4E414D
      450100000000}
    Session = oraTSes
    Left = 244
    Top = 293
  end
  object dsVetka: TDataSource
    DataSet = oraVetka
    Left = 276
    Top = 293
  end
  object oraPotreb: TOracleDataSet
    SQL.Strings = (
      'select id,'
      '       short_name'
      '  from kls_predpr  WHERE is_block<>1'
      'order by short_name')
    QBEDefinition.QBEFieldDefs = {
      030000000200000002000000494401000000000A00000053484F52545F4E414D
      450100000000}
    Session = oraTSes
    Left = 244
    Top = 324
  end
  object dsPotreb: TDataSource
    DataSet = oraPotreb
    Left = 276
    Top = 324
  end
  object oraPoluch: TOracleDataSet
    SQL.Strings = (
      'select id,'
      '       short_name'
      '  from kls_predpr  WHERE is_block<>1'
      'order by short_name'
      '')
    QBEDefinition.QBEFieldDefs = {
      030000000200000002000000494401000000000A00000053484F52545F4E414D
      450100000000}
    Session = oraTSes
    Left = 412
    Top = 249
  end
  object dsPoluch: TDataSource
    DataSet = oraPoluch
    Left = 460
    Top = 257
  end
  object oraUslOpl: TOracleDataSet
    SQL.Strings = (
      'select id,'
      '       name'
      '  from kls_usl_opl'
      'order by kol_dn,name')
    QBEDefinition.QBEFieldDefs = {
      03000000020000000200000049440100000000040000004E414D450100000000}
    Session = oraTSes
    Left = 247
    Top = 238
  end
  object dsUslOpl: TDataSource
    DataSet = oraUslOpl
    Left = 279
    Top = 238
  end
  object oraOrg_Stru: TOracleDataSet
    SQL.Strings = (
      '-- '#1060#1080#1083#1080#1072#1083#1099
      'SELECT * FROM ORG_STRUCTURE WHERE PLAN_REAL<>0'
      'ORDER BY plan_real')
    QBEDefinition.QBEFieldDefs = {
      
      494E5F584C530100000000}
    Session = oraTSes
    Left = 246
    Top = 353
  end
  object dsOrg_Stru: TDataSource
    DataSet = oraOrg_Stru
    Left = 281
    Top = 353
  end
  object oraQUpd: TOracleQuery
    SQL.Strings = (
      'UPDATE month_all SET'
      
        '            (APPL_TAG,kls_predpr_id, kls_dog_id, kls_prod_id, da' +
        'te_dog, request,kol,speed_kol,speed_ves,'
      
        '             date_plan, stan_id, input_date, input_number, payfo' +
        'rm_id,'
      
        '             load_abbr, vetka_id, poluch_id, potreb_id,usl_opl_i' +
        'd,orgstru_id,price,'
      
        #9'poluch_name,potreb_name,kls_planstruid,is_agent,status_zakaz_id' +
        ',client_number,client_date,prich,gr4,BEGIN_DATE'
      '            ) ='
      
        '     (SELECT '#39'MASTER'#39', :kls_predpr_id, :kls_dog_id, :kls_prod_id' +
        ', :date_dog, :request,:kol,:speed_kol,:speed_ves,'
      
        '             :date_plan, :stan_id, TRUNC(:input_date), :input_nu' +
        'mber, :payform_id,'
      
        '             :load_abbr, :vetka_id, :poluch_id, :potreb_id,:usl_' +
        'opl_id,:orgstru_id,:price,'
      
        #9':poluch_name,:potreb_name,:kls_planstruid,:is_agent,:status_zak' +
        'az_id,:client_number,:client_date,:prich,:gr4,:BEGIN_DATE FROM d' +
        'ual'
      '            )'
      'WHERE id=:ID')
    Session = oraTSes
    Variables.Data = {
      
      444154450C0000000000000000000000}
    Left = 494
    Top = 381
  end
  object oraPlanStru: TOracleDataSet
    SQL.Strings = (
      '-- '#1055#1086#1079#1080#1094#1080#1080' '#1087#1083#1072#1085#1072' '#1087#1086#1089#1090#1072#1074#1086#1082
      'Select id,name,region_id from v_plan_stru where has_child=0'
      'and end_date>=:DATE_PLAN')
    Variables.Data = {
      03000000010000000A0000003A444154455F504C414E0C000000000000000000
      0000}
    QBEDefinition.QBEFieldDefs = {
      03000000030000000200000049440100000000040000004E414D450100000000
      09000000524547494F4E5F49440100000000}
    Session = oraTSes
    Left = 246
    Top = 393
    object oraPlanStruID: TFloatField
      FieldName = 'ID'
      Required = True
    end
    object oraPlanStruNAME: TStringField
      FieldName = 'NAME'
      Size = 100
    end
    object oraPlanStruREGION_ID: TFloatField
      FieldName = 'REGION_ID'
    end
  end
  object dsPlanstru: TDataSource
    DataSet = oraPlanStru
    Left = 281
    Top = 393
  end
  object q_status_zakaz: TOracleDataSet
    SQL.Strings = (
      'SELECT ID,NAME  FROM kls_status_zakaz'
      'WHERE 1=1'
      'ORDER BY id')
    QBEDefinition.QBEFieldDefs = {
      03000000020000000200000049440100000000040000004E414D450100000000}
    Session = oraTSes
    Left = 372
    Top = 348
  end
  object ds_status_zakaz: TDataSource
    DataSet = q_status_zakaz
    Left = 320
    Top = 346
  end
  object oraQInsRow: TOracleQuery
    SQL.Strings = (
      'INSERT INTO month_all_row'
      
        '            (MONTH_ALL_ID, SORTBY, CLIENT_NUMBER, CLIENT_DATE, I' +
        'NPUT_NUMBER, INPUT_DATE, REQUEST, KOL, '
      
        #9#9#9' SPEED_VES, SPEED_KOL, KLS_PREDPR_ID, KLS_DOG_ID, DATE_DOG, K' +
        'LS_PROD_ID, STAN_ID, PRICH, '
      
        #9#9#9' PAYFORM_ID, LOAD_ABBR, VETKA_ID, GR4, POLUCH_ID, POTREB_ID, ' +
        'USL_OPL_ID, ORGSTRU_ID, PRICE, '
      #9#9#9' POLUCH_NAME, POTREB_NAME, KLS_PLANSTRUID,'
      #9#9#9' STATUS_ZAKAZ_ID, STAN_OLD_ID,BEGIN_DATE'
      '            )'
      
        '     VALUES (:MONTH_ALL_ID, :SORTBY, :CLIENT_NUMBER, :CLIENT_DAT' +
        'E, :INPUT_NUMBER, :INPUT_DATE, :REQUEST, :KOL,'
      
        #9'         :SPEED_VES, :SPEED_KOL, :KLS_PREDPR_ID, :KLS_DOG_ID, :' +
        'DATE_DOG, :KLS_PROD_ID, :STAN_ID, :PRICH,'
      
        #9#9#9' :PAYFORM_ID, :LOAD_ABBR, :VETKA_ID, :GR4, :POLUCH_ID, :POTRE' +
        'B_ID, :USL_OPL_ID, :ORGSTRU_ID, :PRICE,'
      #9#9#9' :POLUCH_NAME, :POTREB_NAME, :KLS_PLANSTRUID, '
      #9#9#9' :STATUS_ZAKAZ_ID, :STAN_OLD_ID,:BEGIN_DATE'
      '            )')
    Session = oraTSes
    Variables.Data = {
      
      0000003A424547494E5F444154450C0000000000000000000000}
    Left = 198
    Top = 37
  end
  object oraQUpdVes: TOracleQuery
    SQL.Strings = (
      'UPDATE month_all SET'
      '            (request,kol,speed_kol,speed_ves ) ='
      
        '     (SELECT request-:request,kol-:kol,speed_kol-:speed_kol,spee' +
        'd_ves-:speed_ves FROM dual'
      '            )'
      'WHERE id=:ID')
    Session = oraTSes
    Variables.Data = {
      0300000005000000080000003A52455155455354040000000000000000000000
      030000003A4944030000000000000000000000040000003A4B4F4C0300000000
      000000000000000A0000003A53504545445F4B4F4C0300000000000000000000
      000A0000003A53504545445F564553040000000000000000000000}
    Left = 566
    Top = 381
  end
end

...
begin
  ...
  Begin
    Application.MessageDlg('Неизвестная структура файла с реестром '+filename);
    exit;
  end;
...
//бежим по первому столбцу и ищем непустую ячейку
  ...
  while currRow < 50 do begin
    ...
    if s = '1' then begin //нашли строку 1
  ...
  //бежим по 3-му столбцу и ищем фразу "Начало налива"
  currRow := 19;
  while currRow < 200 do begin
    s := VarToStr(sheet.Cells[currRow, 3]);
    if s = 'Начало налива' then begin //нашли подвал отчета
      break;        //начинаем заниматься более серьезными делами
    end;
    inc(currRow);
  end;

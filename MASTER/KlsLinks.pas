...
begin
  ...
  t_kls_right.SQL.Add(WhereStrRight+' 1=1 '); // Для ограничения текущей записи
  t_kls_right.SQL.Add(' AND 1=1 '); // Это для фильтра

  if t_kls_stru_rightSTART_ORDER.AsString<>'' then
    t_kls_right.SQL.Add('ORDER BY '+t_kls_stru_rightSTART_ORDER.AsString) // Это для ORDER BY
  else
    t_kls_right.SQL.Add(''); // Это для ORDER BY
  ...
  begin
    If t_field_stru_rightFIELD_KIND.AsInteger=1 Then //Lookup-поле
    Begin
  ...
  begin
    ...
    begin
      ...
      End;
      If t_field_stru_rightFIELD_KIND.AsInteger=2 Then // Checkbox-поле
      Begin
        Checkboxes:=true;
...
{        gridRightList.FooterRowCount:=1;
  ...
  t_kls_left.SQL.Add(WhereStrLeft+' 1=1 '); // Для ограничения текущей записи
  ...
  t_kls_left.SQL.Add(' AND 1=1 '); // Это для фильтра

  if t_kls_stru_leftSTART_ORDER.AsString<>'' then
    t_kls_left.SQL.Add('ORDER BY '+t_kls_stru_leftSTART_ORDER.AsString) // Это для ORDER BY
  else
    t_kls_left.SQL.Add(''); // Это для ORDER BY
  ...
  begin
    ...
    begin
      If t_field_stru_leftFIELD_KIND.AsInteger=1 Then // Lookup-поле
      Begin
      ...
      End;
      If t_field_stru_leftFIELD_KIND.AsInteger=2 Then // Checkbox-поле
      Begin
        Checkboxes:=true;
        DblClickNextVal:=true;
        KeyList.Add('1');
...
begin
  ...
  Begin
    ...
    if rb1_1.Checked then // Связь только 1 к 1
    Begin
      if not t_link_left.EOF or not t_link_right.EOF then
      Begin
        Application.MessageBox(PChar('Для '+lbLeft.Caption+' возможна связь с '+lbRight.Caption+' только в режиме ОДИН к ОДНОМУ'),'Внимание!',MB_OK);
        Exit;
      end;
    End;

    If NOT LinkFound then
    if rbM_1.Checked then // Связь только M к 1
    Begin
      if not t_link_left.EOF then
      Begin
        Application.MessageBox(PChar('Для '+lbLeft.Caption+' возможна связь с '+lbRight.Caption+' только в режиме МНОГИЕ к ОДНОМУ'),'Внимание!',MB_OK);
        Exit;
      end;
    End;

    if Application.MessageBox(PChar('Связать '+t_kls_left.FieldByName(t_kls_stru_leftNAME_FIELD.AsString).AsString+
                 ' с '+t_kls_right.FieldByName(t_kls_stru_rightNAME_FIELD.AsString).AsString+' ?'),'Внимание!',MB_YESNO)=IDYES then
    try
      if (TableLeft=t_linksTABLE_LINK.AsString) OR (TableRight=t_linksTABLE_LINK.AsString) then
        // Таблица связи - одна из таблиц. Нельзя добавлять и удалять из нее записи
      Begin
        ...
        Begin // Обновляем запись в таблице связи
...
//        q_tmp.SQL.SaveToFile('c:\tmp\1.sql');
      ...
      Else
      Begin
        If NOT LinkFound then // связь не установлена - добавляем
        Begin
        ...
        End
        Else
        Begin // связь установлена - обновляем
...
begin
  ...
  if Application.MessageBox(PChar('Разъединить '+t_kls_left.FieldByName(t_kls_stru_leftNAME_FIELD.AsString).AsString+
               ' с '+t_link_left.FieldByName('NAME').AsString+' ?'),'Внимание!',MB_YESNO)=IDYES then
  try
    DelCurrentLink;
  finally
    acRefreshRightExecute(Sender);

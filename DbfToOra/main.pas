  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        End
        Else
          IF (SRC_TYPE='DOA') or (SRC_TYPE='ORACLE') Then
          begin
            LoadParam.SRC_Where  := LoadParam.SRC_Where + '('+LoadParam.Date_field + '>= TO_DATE(''' +IntToStr(ExtractMonth(DT_Beg))+'/'+
                        IntToStr(ExtractDay(DT_Beg))+'/'+IntToStr(ExtractYear(DT_Beg))+ ''',''mm/dd/yyyy'')) AND ('+
                        LoadParam.Date_field + '<= TO_DATE('''+IntToStr(ExtractMonth(DT_End))+'/'+
                        IntToStr(ExtractDay(DT_End))+'/'+IntToStr(ExtractYear(DT_End))+ ''',''mm/dd/yyyy''))';
          End
          Else
          Begin
            LoadParam.SRC_Where  := LoadParam.SRC_Where + '('+LoadParam.Date_field + '>= #' +IntToStr(ExtractMonth(DT_Beg))+'/'+
...
Begin
  ...
  try
    ...
    Begin
      ...
      cmm_DOA_SRC_Select.SQL.SaveToFile('c:\src_cnt.sql');
      ...
      begin
        ...
        cmm_ADO_SRC_Select.SQL.SaveToFile('c:\src_cnt.sql');
    ...
    Begin
      ...
      If (SRC_Where <> '') then SelStr:=SelStr+' WHERE ' + SRC_Where;
      cmm_DOA_SRC_Select.SQL.Add(SelStr);
      cmm_DOA_SRC_Select.SQL.SaveToFile('c:\src_sel.sql');
      cmm_DOA_SRC_Select.Execute;
    ...
    Else
      ...
      Begin
        ...
        If (SRC_Where <> '') then SelStr:=SelStr+' WHERE ' + SRC_Where;
        cmm_ADO_SRC_Select.SQL.Add(SelStr);
        cmm_ADO_SRC_Select.SQL.SaveToFile('c:\src_sel.sql');
        cmm_ADO_SRC_Select.Open;
        FieldsCount:=cmm_ADO_SRC_Select.Fields.Count;
      End;

    { выбираем список полей таблицы-источника}
    IF FlagViewSQL Then WriteToLog('Результат','Кол-во полей в таблице '+SRC_Table+' - '+IntToStr(FieldsCount),FlagMGRSRFind,ID,Filename);
    IsOk:=true;
    str_InsertTo:='';
    ...
    IF FlagViewSQL Then WriteToLog('Результат',str_InsertTo,FlagMGRSRFind,ID,Filename);
...
{        cmm_DOA_SRC_Select.First;}
      ...
      While Not IsEof do
      Begin
        IF FlagViewSQL Then WriteToLog('Результат','Перебираем значения полей',FlagMGRSRFind,ID,Filename);
        str_Insert := '';
        For i := 1 To FromFields.Count do
        Begin
          IF (SRC_TYPE='DOA') OR (SRC_TYPE='DPL') OR (SRC_TYPE='DAI') Then
...
{                    dpl_DOA_Insert.Columns[i-1].SetData(dpl_Row, @STRING(FieldsValue[dpl_Row,i])[1], Length(STRING(FieldsValue[dpl_Row,i])));}
            ...
            Else
              ...
              Begin
                ...
                IF FlagViewSQL Then WriteToLog('Результат',str_Insert,FlagMGRSRFind,ID,Filename);
              end;
          End;
        end;
